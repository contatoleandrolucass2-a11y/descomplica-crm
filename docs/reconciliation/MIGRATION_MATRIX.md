...
- As quatro migrations locais posteriores à captura remota também aparecem no
...
|    22 | `20260809144143_qlik_rls_contract_hardening`                   |  Sim  |  Não   | `262eb87f232f6e0e3cf670bd742669899271a03e543d493da58049a443052660` | —                                                                  | Preserva developments, revoga ACL direta, remove a RPC Qlik anônima e cria ingestão interna/leitura escopada. | Somente local; destrutiva para o caller Qlik ativo          | **Bloqueada remotamente:** identificar caller/owner e aprovar/validar relay antes; aplicar agora interrompe a carga observada. |
...
históricos do banco remoto. As versões 21–24 são somente locais; nenhuma delas
foi aplicada ao projeto remoto.
...
Aplicar as sete migrations somente locais não converge o banco e não constitui
uma sequência remota segura:
...
- `20260809144143` vem antes das migrations v3 na cadeia local e revoga a ACL,
  remove a RPC legada e, conforme o próprio arquivo declara, interrompe o caller
  anônimo ativo se for aplicada antes do relay.

Portanto, não existe hoje uma Fase A remotamente aplicável que alcance
`20260809181422`/`20260809181424` e preserve o caller Qlik. A migration
`20260809144143` deve permanecer bloqueada ou ser separada/reversionada em ponte
aditiva e hardening final posterior. O hardening final exige caller e owner
identificados, relay aprovado e validado em paralelo, duas janelas observadas e
autorização remota explícita. Nenhum desses passos de cutover ocorreu neste gate.

O plano seguro para o próximo gate é:

1. identificar o caller Qlik ativo, atribuir owner e aprovar um relay CRM-owned;
2. criar markers históricos explicitamente não autoritativos para as quatro
   versões remotas, sem copiar verifier, grants ou fórmula;
3. separar a ponte aditiva do hardening destrutivo hoje reunido em
   `20260809144143`, sem mudar o caller durante a instalação da fundação;
4. executar reset completo, pgTAP de ACL/RLS e comparação do schema local;
5. obter backup remoto e restauração isolada comprovados;
6. revisar o dry-run completo, incluindo as sete versões somente locais, os
   markers e a convergência;
7. validar relay e dual-write por duas janelas antes de agendar a revogação;
8. somente em autorização posterior, aplicar pelo mecanismo oficial e validar
