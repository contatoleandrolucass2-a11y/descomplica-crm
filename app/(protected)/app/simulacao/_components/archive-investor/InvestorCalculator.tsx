...
import { useEffect, useId, useMemo, useRef, useState, type KeyboardEvent as ReactKeyboardEvent, type ReactNode, type Ref } from "react";
...
const INVESTOR_TOUR_STEPS = [
  {
    ...
    description: "A página segue quatro blocos: estoque, planos prontos, proposta personalizada e resultado. O guia mostra onde agir e o que conferir em cada um.",
    tip: "Avançar no guia não altera valores. Você pode voltar, fechar e reiniciar quando quiser.",
    checklist: ["Siga os itens 01 a 04", "Use Anterior para revisar", "Conclua somente depois da validação"],
  },
  {
    target: "information",
    eyebrow: "Ajuda em cada etapa",
    title: "Consulte os ícones de informação",
    description: "Os ícones de informação explicam regras, limites e ações do campo ao lado. Passe o cursor ou use o foco para consultar; clique para manter a explicação aberta.",
    tip: "Clique em qualquer área fora da explicação para fechá-la.",
    checklist: ["Procure o ícone ao lado do campo", "Leia a orientação antes de preencher", "Use a ajuda sempre que tiver dúvida"],
  },
  {
    target: "filters",
    eyebrow: "Encontre o imóvel",
    title: "Defina o perfil desejado",
    description: "Combine Incorporadora, Empreendimento, Região, Planta e Valor do Imóvel. Cada escolha atualiza as opções e o total de unidades disponíveis.",
    tip: "Vagas de garagem avulsas não são comercializadas nesta modalidade. Use Limpar filtros para recomeçar.",
    checklist: ["Combine quantos filtros precisar", "Confira o total encontrado", "Ajuste até chegar ao perfil correto"],
  },
  {
    target: "inventory",
    eyebrow: "Escolha uma unidade",
    title: "Escolha a unidade correta",
    description: "Revise produto, metragem, entrega, planta e valor. Selecione a unidade pela linha ou pelo botão circular da primeira coluna.",
    tip: "A unidade escolhida define valor, prazo da obra e todas as datas. Sem ela, os próximos passos ficam bloqueados.",
    checklist: ["Confirme empreendimento e produto", "Confira metragem, planta e entrega", "Selecione apenas quando os dados estiverem corretos"],
  },
  {
    target: "sort",
    eyebrow: "Organize a comparação",
    title: "Ordene as unidades por valor",
    description: "Escolha Menor para o maior ou Maior para o menor. A ordenação muda somente a sequência da lista e facilita encontrar a faixa de preço desejada.",
    tip: "A unidade selecionada conserva os mesmos dados; somente a posição da lista muda.",
    checklist: ["Escolha a direção da ordenação", "Compare unidades próximas de preço", "Confirme o valor antes de seguir"],
  },
  {
    target: "scenarios",
    eyebrow: "Compare os planos prontos",
    title: "Compare as oito propostas prontas",
    description: "Selecione o plano de 18 ou 24 parcelas e escolha entre as quatro combinações com ou sem sinais e intermediárias. Cada opção abre a composição completa no livro-caixa.",
    tip: "Analise uma opção por vez e alterne entre os planos sem perder as oito possibilidades.",
    checklist: ["Escolha o prazo desejado", "Compare entrada e pagamentos adicionais", "Observe o valor e o início das mensais"],
  },
  {
    target: "proposal",
    eyebrow: "Monte a proposta",
    title: "Monte a proposta personalizada",
    description: "A entrada inicia em 10% e continua editável. Se necessário, adicione sinais, intermediárias ou desconto e escolha a quantidade de parcelas permitida.",
    tip: "Acompanhe percentuais, vencimentos, limites e avisos em cada linha; valores fora da regra são destacados.",
    checklist: ["Entrada mínima de 6%", "Entrada total mínima de 10%", "Sinais e intermediárias devem respeitar os avisos"],
  },
  {
    target: "result",
    eyebrow: "Leia o resultado",
    title: "Confira a composição final",
    description: "Confira valor do imóvel, entrada, sinais válidos, intermediárias válidas, saldo, quantidade de parcelas, valor mensal e datas inicial e final.",
    tip: "O selo informa se a proposta está aprovada ou se ainda precisa de ajuste.",
    checklist: ["Reconcilie cada pagamento", "Confira saldo e parcela mensal", "Valide primeira e última mensal"],
  },
  {
    target: "documents",
    eyebrow: "Documentação do cliente",
    title: "Consulte os documentos necessários",
    description: "Use Doc Pessoa Física ou Doc Pessoa Jurídica para abrir a relação adequada ao tipo de cliente antes de formalizar a proposta.",
    tip: "A lista documental não substitui a conferência do gerente ou da área responsável.",
    checklist: ["Identifique o tipo de cliente", "Abra a lista correspondente", "Confirme todos os documentos antes de imprimir"],
  },
  {
    target: "audit",
    eyebrow: "Valide a proposta",
    title: "Faça a última conferência",
    description: "Abra a auditoria, leia cada condição e corrija todos os itens reprovados. Só apresente ou imprima depois de confirmar regras, valores, datas e documentação.",
    tip: "Imprima somente depois de confirmar unidade, valores, datas, documentação e o selo de proposta dentro da regra.",
    checklist: ["Nenhuma condição pode ficar reprovada", "Confira a documentação do cliente", "Imprima somente a versão validada"],
  },
] as const;

const ASSOCIATIVE_TOUR_ORDER = ["welcome", "information", "filters", "inventory", "sort", "qualification", "proposal", "result", "documents"] as const;

const ASSOCIATIVE_TOUR_STEPS = [...INVESTOR_TOUR_STEPS.filter((step) => step.target !== "audit" && step.target !== "scenarios"), {
  target: "qualification",
  eyebrow: "Perfil do financiamento",
  title: "Responda as 3 perguntas na ordem",
  description: "Primeiro digite a renda familiar. Depois confira MCMV ou SBPE. Por último, responda se é o primeiro imóvel. Cada resposta libera a próxima.",
  ...
  if (step.target === "proposal") return {
    ...step,
    eyebrow: "Monte a proposta",
    title: "Preencha os recursos e pagamentos",
    description: "Confira B.A. e folga e informe primeiro o Financiamento. Depois ajuste Subsídio, FGTS, Cheque Moradia, Entrada, Sinais, Anuais e quantidade de parcelas.",
    tip: "O financiamento libera os cálculos; saldo e parcela corrigida são atualizados a cada valor informado.",
    checklist: ["Confira as deduções da unidade", "Informe recursos e entrada", "Revise sinais, anuais e parcelas"],
  };
  if (step.target === "result") return {
    ...step,
    eyebrow: "Leia o resultado",
    title: "Confira a composição final",
    description: "Confira valor real da venda, recursos, entrada, sinais, anuais corrigidas, saldo parcelado e parcela corrigida.",
    tip: "Corrija qualquer item reprovado antes de apresentar a proposta.",
    checklist: ["Reconcilie recursos e pagamentos", "Confira o saldo parcelado", "Valide a parcela corrigida"],
  };
  if (step.target === "documents") return {
    ...step,
    eyebrow: "Documentação associativa",
    title: "Consulte os documentos da Pessoa Física",
    description: "Abra Doc Pessoa Física para conferir identificação, estado civil, comprovantes de renda e documentos dos dependentes antes de formalizar a proposta associativa.",
    tip: "A modalidade associativa não aceita Pessoa Jurídica; a análise de crédito é realizada pela CAIXA.",
    checklist: ["Abra Doc Pessoa Física", "Confira renda e dependentes", "Imprima somente a versão validada"],
  };
  return step;
}).sort((first, second) => {
  return ASSOCIATIVE_TOUR_ORDER.indexOf(first.target) - ASSOCIATIVE_TOUR_ORDER.indexOf(second.target);
});

const DIRECT_TABLE_TOUR_STEPS = [
  { target: "welcome", eyebrow: "Vamos começar", title: "Monte a proposta como um quebra-cabeça", description: "Primeiro você escolhe o imóvel. Depois informa a renda, escolhe um modelo pronto, confere os pagamentos e valida o resultado. O simulador faz as contas; você confirma se os dados estão corretos.", tip: "O guia apenas explica e aponta os lugares da tela. Ele não muda nenhum valor sozinho.", checklist: ["Escolha o imóvel", "Monte os pagamentos", "Confira antes de imprimir"] },
  ...
  { target: "income", eyebrow: "Passo 4 · informar a renda", title: "Digite a renda mensal real do cliente", description: "A renda precisa ser maior que zero. Enquanto ela não for informada, a primeira opção e todo o fluxo editável ficam visíveis, mas congelados. Depois do preenchimento, o sistema libera os controles e compara a parcela pós-chaves com a renda.", tip: "O limite de comprometimento é 40%. A simulação orienta o atendimento, mas não substitui a análise de crédito interna.", checklist: ["Use a renda mensal real", "Digite um valor maior que zero", "Aguarde a liberação automática"] },
...
const STANDARD_SCENARIO_OPTIONS = {
  ...
  C2: { plan: 18, title: "Proposta com sinais e sem intermediárias", description: "Entrada de 6% complementada por 3 sinais, chegando aos 10%." },
  C3: { plan: 18, title: "Proposta com sinais e com intermediárias", description: "Entrada de 6% complementada por 3 sinais e até 3 intermediárias." },
  C4: { plan: 18, title: "Proposta sem sinais e com intermediárias", description: "Entrada integral de 10% e até 3 intermediárias." },
  C5: { plan: 24, title: "Proposta sem sinais e sem intermediárias", description: null },
  C6: { plan: 24, title: "Proposta com sinais e sem intermediárias", description: "Entrada de 17% complementada por 3 sinais de 1%, chegando aos 20%." },
  C7: { plan: 24, title: "Proposta com sinais e com intermediárias", description: "Entrada de 17%, 3 sinais de 1% e até 4 intermediárias." },
  C8: { plan: 24, title: "Proposta sem sinais e com intermediárias", description: "Entrada integral de 20% e até 4 intermediárias." },
} as const;

const ASSOCIATIVE_SCENARIO_OPTIONS = {
  C1: { plan: 84, title: "Proposta sem sinais e sem anuais", description: "Entrada ajustada à classificação, sem regra percentual fixa." },
  C2: { plan: 84, title: "Proposta com sinais e sem anuais", description: "Entrada e 3 sinais iguais, respeitando o mínimo de R$ 150,00." },
  C3: { plan: 84, title: "Proposta sem sinais e com anuais", description: "Anuais até 50% da renda e entrada ajustada ao saldo." },
  C4: { plan: 84, title: "Proposta com sinais e com anuais", description: "Anuais até 50% da renda; entrada e 3 sinais iguais." },
} as const;
...
function sameCurrencyAmount(first: number, second: number) {
...
function MoneyInput({ id, value, onChange, label, describedBy, invalid = false, disabled = false, max, limitToMax = false, inputRef }: { id?: string; value: string; onChange: (value: string) => void; label: string; describedBy?: string; invalid?: boolean; disabled?: boolean; max?: number; limitToMax?: boolean; inputRef?: Ref<HTMLInputElement> }) {
  return <input
    id={id}
    ref={inputRef}
    ...
    onChange={(event) => {
      ...
      onChange(limitToMax && max != null && currencyInputNumber(parsedValue) > max
        ? Math.max(0, max).toFixed(2)
        : parsedValue);
    }}
    placeholder="0,00"
    data-max={max}
...
function AssociativeMoneyControl({ label, value, onChange, describedBy, invalid = false, disabled = false, max, limitToMax = false, inputRef }: { label: string; value: string; onChange: (value: string) => void; describedBy?: string; invalid?: boolean; disabled?: boolean; max?: number; limitToMax?: boolean; inputRef?: Ref<HTMLInputElement> }) {
  return <div className="investor-direct-editable-value investor-associative-line-control">
    <span aria-hidden="true">R$</span>
    <MoneyInput inputRef={inputRef} label={label} describedBy={describedBy} invalid={invalid} disabled={disabled} max={max} limitToMax={limitToMax} value={value} onChange={onChange} />
  </div>;
}

function AssociativeStepGuide({ title, description, tone = "guidance" }: { title: string; description: string; tone?: "guidance" | "rejected" }) {
  return <aside className={`investor-associative-step-guide ${tone}`} role="status" aria-live="polite" aria-atomic="true">
...
function AssociativeMoneyValue({ value, label }: { value: number; label: string }) {
  return <div className="investor-direct-editable-value investor-associative-line-control investor-associative-readonly-value" aria-label={`${label}: ${money.format(value)}`}>
    <span aria-hidden="true">R$</span>
    <strong>{currencyInput.format(Math.max(0, value))}</strong>
  </div>;
}

function DirectReadyLedgerValue({ label, amount, text, help, action }: { label: string; amount?: number; text: string; help: string; action?: ReactNode }) {
  return <div className="investor-associative-value-help investor-direct-ready-value-help">
    <div className={`investor-direct-editable-value investor-associative-line-control investor-associative-readonly-value${amount == null ? " is-text" : ""}`} aria-label={`${label}: ${amount == null ? text : money.format(amount)}`}>
      {amount == null ? <strong>{text}</strong> : <><span aria-hidden="true">R$</span><strong>{currencyInput.format(Math.max(0, amount))}</strong></>}
    </div>
...
function DirectEditableAccountRow({ number, operator, label, date, dateLabel, meta, help, calculation, result, cornerAction, total = false, invalid = false, action = false, tourTarget, twoColumn = false, fieldState, rowClassName = "", leadingAction, sideGuidance, disabled = false }: DirectEditableAccountRowProps) {
  ...
  return <li className={`${total ? "total" : ""}${invalid ? " invalid" : ""}${fieldState ? ` field-${fieldState}` : ""}${disabled ? " is-stage-locked" : ""}${sideGuidance ? " has-side-guidance" : ""}${rowClassName ? ` ${rowClassName}` : ""}`} value={number} role={number ? undefined : "presentation"} data-tour={tourTarget} aria-disabled={disabled || undefined} aria-invalid={invalid || undefined}>
    ...
    <div className="investor-direct-step-content">
      ...
      {twoColumn ? <div className="investor-direct-step-formula investor-direct-step-composition">
        <div className="investor-direct-step-calculation investor-associative-calculation-line"><span className="investor-associative-ledger-operator" aria-hidden="true">{operator ?? ""}</span>{operatorLabel ? <span className="sr-only">{operatorLabel}</span> : null}<div>{calculation}</div></div>
        {result != null ? <div className={`investor-direct-step-outcome${action ? " has-action" : ""}`}>{result}</div> : null}
      </div> : <>
        <div className="investor-direct-step-formula">{calculation}</div>
        {result != null ? <div className={`investor-direct-step-result${action ? " has-action" : ""}`}>{result}</div> : null}
      </>}
    </div>
    {cornerAction ? <div className="investor-associative-corner-action">{cornerAction}</div> : null}
  ...
  return <article className={`investor-direct-comparison-card${active ? " is-active" : ""}`} aria-labelledby={titleId} data-selected={active || undefined}>
    ...
    <div className="investor-direct-comparison-ledger" role="table" aria-label="Livro-caixa da composição atual">
      ...
      <DirectComparisonLedgerRow
        ...
        value={preKeysAvailable && sameCurrencyAmount(flow.custom.installmentValue, flow.custom.lastInstallmentValue) ? flow.custom.installmentValue : 0}
        displayValue={preKeysDeadlineInsufficient ? "Prazo insuficiente" : preKeysAvailable && !sameCurrencyAmount(flow.custom.installmentValue, flow.custom.lastInstallmentValue) ? preKeysPaymentSummary : preKeysAvailable ? undefined : "Dispensadas"}
...
function InvestorScenarioComparisonCard({ scenario, option, optionNumber, valueReal, baseDate }: {
  scenario: ReturnType<typeof calculateInvestorFlow>["standardScenarios"][number];
  option: { plan: number; title: string; description: string | null };
  optionNumber: number;
  valueReal: number;
  baseDate: string;
}) {
  const titleId = useId();
  const signalDates = scenario.signals.map((signal: { date: string }) => formatDate(signal.date));
  const intermediaryDates = scenario.intermediaryDates.map((paymentDate: string) => formatDate(paymentDate));
  const signalDetail = scenario.signals.length > 0
    ? `${scenario.signals.length} sinais · pagamentos em ${signalDates.join(", ")}`
    : "Sem sinais nesta proposta";
  const intermediaryDetail = scenario.intermediaryCount > 0
    ? `${scenario.intermediaryCount} intermediárias de até 5% · pagamentos em ${intermediaryDates.join(", ")}`
    : "Sem intermediárias nesta proposta";
  const paymentSummary = installmentSummary(
    scenario.installments,
    scenario.installmentValue,
    scenario.lastInstallmentValue,
  );
  const lastInstallmentDate = scenario.monthlyDates.at(-1) ?? "";

  return <article className="investor-direct-comparison-card investor-standard-comparison-card is-active" aria-labelledby={titleId} data-selected="true">
    <header className="investor-direct-comparison-heading">
      <div>
        <div className="investor-direct-comparison-option-line"><span>{`Base: plano ${option.plan} · opção ${String(optionNumber).padStart(2, "0")}`}</span></div>
        <h4 id={titleId}>Composição atual calculada</h4>
        <p>{[
          `Entrada de ${percent.format(scenario.entryRate)}`,
          scenario.signals.length > 0 ? `${scenario.signals.length} sinais somam ${percent.format(scenario.signalTotal / valueReal)}` : "sem sinais",
          scenario.intermediaryCount > 0 ? `${scenario.intermediaryCount} intermediárias somam ${percent.format(scenario.intermediaryTotal / valueReal)}` : "sem intermediárias",
        ].join(" · ")}</p>
      </div>
      <div className={`investor-direct-comparison-status ${scenario.available ? "approved" : "pending"}`}>
        <small>Resultado</small>
        <strong>{scenario.available ? "VALIDADO" : "INDISPONÍVEL"}</strong>
        <span>{scenario.available ? `Plano em ${scenario.installments} parcelas` : "Prazo da obra insuficiente"}</span>
      </div>
    </header>
    <div className="investor-direct-comparison-ledger" role="table" aria-label={`${option.title}: composição completa`}>
      <DirectComparisonLedgerRow label="Valor real da venda" detail="Base usada nesta proposta pronta" operator="=" value={valueReal} />
      <DirectComparisonLedgerRow label="Entrada" detail={`Pagamento em ${formatDate(baseDate)} · ${percent.format(scenario.entryRate)} do valor`} operator="−" value={scenario.entry} />
      <DirectComparisonLedgerRow label="Sinais" detail={signalDetail} operator="−" value={scenario.signalTotal} muted={scenario.signals.length === 0} />
      <DirectComparisonLedgerRow label="Intermediárias" detail={intermediaryDetail} operator="−" value={scenario.intermediaryTotal} muted={scenario.intermediaryCount === 0} />
      <DirectComparisonLedgerRow label="Saldo parcelado" detail="Valor restante depois da entrada, sinais e intermediárias" operator="=" value={scenario.balance} />
      <DirectComparisonLedgerRow
        label={`${scenario.installments} parcelas mensais`}
        detail={`1ª em ${formatDate(scenario.firstInstallmentDate)}${lastInstallmentDate ? ` · última em ${formatDate(lastInstallmentDate)}` : ""}`}
        operator="÷"
        value={sameCurrencyAmount(scenario.installmentValue, scenario.lastInstallmentValue) ? scenario.installmentValue : undefined}
        displayValue={sameCurrencyAmount(scenario.installmentValue, scenario.lastInstallmentValue) ? undefined : paymentSummary}
        emphasized
      />
    </div>
    <footer className="investor-direct-comparison-footer">
      <span>Primeira mensal <strong>{formatDate(scenario.firstInstallmentDate)}</strong></span>
      <span>Última mensal <strong>{formatDate(lastInstallmentDate)}</strong></span>
    </footer>
  </article>;
}
...
function AssociativeEditableAccountRow({ label, meta, calculation, ...props }: Omit<DirectEditableAccountRowProps, "twoColumn">) {
  ...
  return <DirectEditableAccountRow
    {...props}
    label={label}
    calculation={<div className="investor-associative-value-help">
      <div className="investor-associative-value-only">{calculation}</div>
      {helpDescription ? <InvestorInfoHint label={label} title={`Entenda ${label}`} description={helpDescription} /> : null}
    </div>}
    twoColumn
  />;
}
...
function installmentSummary(count: number, regularValue: number, lastValue: number) {
  ...
  if (count === 1 || sameCurrencyAmount(regularValue, lastValue)) return `${count}x de ${money.format(regularValue)}`;
  return `${count - 1}x de ${money.format(regularValue)} + última de ${money.format(lastValue)}`;
}

function installmentSummaryFromBalance(count: number, balance: number) {
  if (count <= 0) return "Dispensadas";
  const balanceInCents = Math.max(0, Math.round(balance * 100));
  const regularInCents = Math.floor(balanceInCents / count);
  const lastInCents = balanceInCents - regularInCents * Math.max(0, count - 1);
  return installmentSummary(count, regularInCents / 100, lastInCents / 100);
}
  ...
  return <aside className="investor-hero-guide" aria-label="Guia completo do simulador" data-tour="welcome">
    <div className="investor-proposal-help-cta">
      ...
      <button
        ...
        onClick={(event) => window.dispatchEvent(new CustomEvent("investor:start-guide", { detail: { trigger: event.currentTarget } }))}
      >Iniciar passo a passo</button>
    </div>
  </aside>;
}
...
function InvestorCommercialLinks() {
  return <>
    <a className="investor-resource-platform" href="https://boravender.app.br/login" target="_blank" rel="noopener noreferrer" aria-label="Abrir Bora Vendas em nova aba">
      <span className="investor-hero-logo bora" aria-hidden="true" /><span><small>Acessar plataforma</small><strong>Bora Vendas</strong></span>
    </a>
    <a className="investor-resource-platform" href="https://direcional.my.site.com/vendas/s/" target="_blank" rel="noopener noreferrer" aria-label="Abrir Salesforce em nova aba">
      <span className="investor-hero-logo salesforce" aria-hidden="true" /><span><small>Acessar plataforma</small><strong>Salesforce</strong></span>
    </a>
  </>;
}
...
function associativeModalityMessage(decision: FinancingDecision) {
  ...
  if (decision.reasonCodes.includes("INCOME_ABOVE_MCMV_LIMIT") && decision.reasonCodes.includes("NOT_FIRST_PROPERTY")) return "SBPE automático: renda acima de R$ 13.000,00 e cliente fora da regra de primeiro imóvel.";
  if (decision.reasonCodes.includes("INCOME_ABOVE_MCMV_LIMIT")) return "SBPE automático: renda familiar acima do limite atual de R$ 13.000,00 do MCMV.";
  if (decision.reasonCodes.includes("NOT_FIRST_PROPERTY")) return "SBPE automático: cliente informou que não se enquadra como primeiro imóvel.";
  if (decision.reasonCodes.includes("PROPERTY_ABOVE_MCMV_LIMIT")) return "SBPE automático: valor do imóvel acima do limite atual de R$ 600.000,00 do MCMV.";
...
function AssociativeQualificationPanel({
  ...
  return <section ref={sectionRef} tabIndex={-1} className={`investor-associative-qualification${completed === 3 ? " is-complete" : ""}${guided ? " is-guided-active" : ""}`} aria-labelledby="investor-associative-qualification-title" data-tour="qualification">
    <header className="investor-section-heading investor-associative-qualification-heading">
      <span>02</span>
      <div><p>Perfil do financiamento</p><h2 id="investor-associative-qualification-title">Responda para liberar o fluxo</h2></div>
    ...
    <div className="investor-associative-qualification-grid">
      ...
      <fieldset className={`investor-associative-question${!modalityReady ? " locked" : firstPropertyReady ? " complete" : " current"}`}>
        ...
        <small role="status" aria-live="polite">{!modalityReady ? "Conclua a etapa anterior" : firstPropertyReady ? "Perfil completo" : "Sim = sem outro imóvel residencial ou financiamento habitacional ativo"}</small>
      </fieldset>
    </div>
  </section>;
}
...
function AssociativeDocumentationPanel({
  ...
  return <><aside className={`investor-associative-documentation ${status}`} aria-labelledby="investor-associative-documentation-title">
    <header>
      <div><span>03</span><div><p>Proposta calculada</p><h3 id="investor-associative-documentation-title">Resumo financeiro</h3></div></div>
      <div className="investor-associative-documentation-actions">
        <button type="button" disabled={!result.ok} aria-haspopup="dialog" aria-controls="investor-associative-documentation-installments" onClick={() => installmentsDialog.current?.showModal()}>Exibir parcelas</button>
    ...
    {result.ok ? <>
      <div className="investor-associative-documentation-summary">
        <section className="investor-associative-documentation-plan" aria-label="Plano sugerido para a documentação">
          ...
          <small>1ª parcela para <b>{formatDate(result.firstInstallmentDate)}</b></small>
          <div><small>Total da documentação</small><strong>{money.format(result.totalCash)}</strong></div>
        </section>
  ...
  >
    <article>
      <header>
        <div><p>Documentação</p><h2 id="investor-associative-documentation-installments-title">Parcelas da documentação</h2></div>
        <form method="dialog"><button type="submit" aria-label="Fechar parcelas da documentação">×</button></form>
      </header>
      {result.ok ? <>
        <div className="investor-direct-amortization-scroll investor-associative-documentation-installment-table-wrap" role="region" tabIndex={0} aria-label="Cronograma das parcelas da documentação">
          <table>
            <caption className="sr-only">Cronograma mensal da documentação</caption>
            <thead><tr><th scope="col">Parcela</th><th scope="col">Vencimento</th><th scope="col">Valor</th></tr></thead>
            <tbody>{documentationInstallments.map((item) => <tr key={item.number}>
              <th scope="row">{String(item.number).padStart(2, "0")}/{result.installments}</th>
              <td><time dateTime={item.paymentDate}>{formatDate(item.paymentDate)}</time></td>
              <td><strong>{money.format(item.value)}</strong></td>
            </tr>)}</tbody>
          </table>
        </div>
      </> : <p className="investor-associative-documentation-installment-empty">Complete a proposta para gerar o cronograma.</p>}
    </article>
  </dialog></>;
}
...
const ASSOCIATIVE_COMMISSION_OPTIONS: Record<AssociativeCommissionChannel, readonly AssociativeCommissionOption[]> = {
...
function AssociativeCommissionPanel({
  ...
  return <aside className={`investor-associative-commission ${commissionReady ? "ready" : "pending"}`} aria-labelledby="investor-associative-commission-title">
    <header>
      <div><span>04</span><div><p>Remuneração comercial</p><h3 id="investor-associative-commission-title">Comissão + Prêmio da venda</h3></div></div>
    </header>
    ...
    <div className="investor-associative-commission-table" aria-live="polite" aria-atomic="true">
      <table>
        ...
        <thead>
          <tr><th scope="col">Composição</th><th scope="col">Base</th><th scope="col">Taxa</th><th scope="col">Resultado</th></tr>
        </thead>
        <tbody>
          <tr>
            ...
            <td><div className="investor-associative-commission-result"><span>Valor da Comissão</span><strong>{commissionReady ? money.format(commissionValue) : "—"}</strong></div></td>
          </tr>
          <tr>
            <th scope="row">Prêmio MKT V.C</th>
            <td>{money.format(awardBase)}</td>
            <td>{hasAward ? percent.format(awardRate) : "—"}</td>
            <td><div className="investor-associative-commission-result"><span>Valor da premiação</span><strong>{hasAward ? money.format(awardValue) : commissionReady ? "Sem premiação" : "—"}</strong></div></td>
          </tr>
        </tbody>
        <tfoot>
          <tr>
            <th scope="row">Total</th>
            <td><strong>{commissionReady ? money.format(totalValue) : "—"}</strong></td>
            <td><span>% total</span></td>
            <td><strong>{commissionReady && totalRate !== null ? percent.format(totalRate) : "—"}</strong></td>
          </tr>
        </tfoot>
      </table>
    </div>
...
function AssociativeApprovalPanel({
  ...
  const adjustmentFor = (flow: AssociativeFlow, suggestion: AssociativeFlowSuggestion | null | undefined) => {
    ...
    if (status === "pending") return { summary: "Complete os campos-chave para calcular este fluxo.", rows: [] };
    if (status === "approved") return { summary: "Nenhum ajuste necessário.", rows: [] };
    if (tierRejectsAll) return { summary: "Selecione um Ranking elegível para comparar este fluxo com limites de aprovação.", rows: [] };
    if (!failure) return { summary: proposalError || "Revise os dados informados neste fluxo.", rows: [] };
    if (suggestion === undefined) return { summary: "Preparando a prévia exata do ajuste…", rows: [] };
    if (!suggestion) return { summary: "Nenhum ajuste automático válido foi encontrado para este fluxo.", rows: [] };
    const signalRows: AssociativeAdjustmentRow[] = suggestion.signals
      .map((value, index) => ({ label: `Sinal ${index + 1}`, date: currentSignalPayments[index]?.date, value, currency: true }))
      .filter((row) => Number(row.value) > 0);
    const annualRows: AssociativeAdjustmentRow[] = suggestion.annuals
      .map((value, index) => ({ label: `Anual ${index + 1}`, date: currentAnnualPayments[index]?.date, value, currency: true }))
      .filter((row) => Number(row.value) > 0);
    return {
      summary: "",
      rows: [
        { label: "Qtd. de parcelas", value: suggestion.installments, featured: true },
        { label: "Entrada sugerida", date: currentEntryDate, value: suggestion.entry, currency: true },
        ...signalRows,
        ...annualRows,
  ...
  const hasApplicableAdjustment = (feedbackTone === "rejected" || feedbackTone === "partial") && !tierRejectsAll && (linearStatus === "rejected" || decreasingStatus === "rejected");
  const adjustmentOptions = [
    ...(linearStatus === "rejected" ? [{ flow: "linear" as const, label: "Fluxo Linear", suggestion: preparedSuggestions.linear, details: adjustmentFor("linear", preparedSuggestions.linear) }] : []),
    ...(decreasingStatus === "rejected" ? [{ flow: "decreasing" as const, label: "Fluxo Decrescente", suggestion: preparedSuggestions.decreasing, details: adjustmentFor("decreasing", preparedSuggestions.decreasing) }] : []),
  ];
  const prepareAndOpenAdjustments = () => {
    ...
    if (decreasingStatus === "rejected") nextSuggestions.decreasing = onBuildFlowSuggestion("decreasing");
  ...
  const applySuggestionAndClose = (flow: AssociativeFlow, suggestion: AssociativeFlowSuggestion | null | undefined) => {
    if (!suggestion) return;
    onApplyFlowSuggestion(flow, suggestion);
    adjustmentsDialogRef.current?.close();
  };
  const releaseLabel = (item: typeof releaseStatus.commission) => item.status === "released"
    ? "LIBERADO"
  ...
  return <section className={`investor-associative-approval ${approval.status}`} aria-labelledby="investor-associative-approval-title">
    <header>
      <div><div><p>Parâmetros de aprovação</p><h3 id="investor-associative-approval-title">Resultado × regra da classificação</h3></div></div>
      <label className={`investor-associative-approval-editable investor-key-field${financingReady && !entryPending && !entryRejected && !tier ? " is-active" : ""}`}><select aria-label="Selecione o Ranking" name="classificacao-associativo" value={tierId} onChange={(event) => onTierChange(event.target.value)}><option value="">Selecione o Ranking</option>{ASSOCIATIVE_APPROVAL_TIERS.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select></label>
    </header>
    {tier ? <>
    <table>
    ...
    >
      <article>
        <header>
          <div><p>Ajuste da proposta</p><h2 id="investor-associative-adjustments-title">Ajustes necessários</h2></div>
          <form method="dialog"><button type="submit" aria-label="Fechar ajustes">×</button></form>
        </header>
        <p id="investor-associative-adjustments-description">Confira a prévia e escolha o fluxo. O sistema nunca diminui a Entrada, os Sinais ou as Anuais já informados: ele apenas acrescenta o que faltar. Ao aplicar, a janela fecha para você conferir o novo resultado.</p>
        <div className="investor-associative-adjustment-options">
          {adjustmentOptions.map((option) => <section key={option.flow} className={`is-${option.flow}`}>
            <header>
              <div><span>{option.label}</span><small>Prévia exata do ajuste</small></div>
              <strong>REPROVADO</strong>
            </header>
            {option.details.summary ? <p className="investor-associative-adjustment-summary">{option.details.summary}</p> : null}
            {option.details.rows.length > 0 ? <dl className="investor-associative-adjustment-ledger">
              {option.details.rows.map((row) => <div key={row.label} className={`${row.featured ? "featured " : ""}${row.date ? "payment-row" : ""}`.trim() || undefined}>
                <dt>{row.label}</dt>
                <span className="payment-date">{row.date ? formatPaymentDate(row.date) : ""}</span>
                <span className="operator" aria-hidden="true">=</span>
                <span className="currency" aria-hidden="true">{row.currency ? "R$" : ""}</span>
                <dd aria-label={row.currency && typeof row.value === "number" ? money.format(row.value) : undefined}>{row.currency && typeof row.value === "number" ? currencyInput.format(row.value) : row.value}</dd>
              </div>)}
            </dl> : null}
            <footer><button type="button" disabled={!option.suggestion} onClick={() => applySuggestionAndClose(option.flow, option.suggestion)}>{option.suggestion ? `Aplicar estes valores no ${option.label}` : "Ajuste automático indisponível"}</button></footer>
          </section>)}
        </div>
      </article>
    </dialog> : null}
    <footer className={`${feedbackTone}${hasApplicableAdjustment ? " has-adjustment-action" : ""}`} role="status" aria-live="polite" aria-atomic="true">
      {hasApplicableAdjustment ? <button
        type="button"
      ...
      >
        ...
        <span>{feedbackLabel}</span>
        <div className="investor-associative-approval-feedback-copy"><small>{feedbackMessage}</small></div>
      </>}
    </footer>
    </> : null}
  </section>;
    ...
    questions: [
      {
        ...
        answer: "É uma modalidade para parcelar o valor do imóvel durante o período de obra, conforme a política comercial interna. O simulador reúne estoque, planos prontos, proposta personalizada, resultado e validação em um único fluxo.",
      },
      {
        question: "Qual é a principal vantagem para o cliente?",
        answer: "A principal vantagem informada nesta modalidade é a não incidência de correção sobre o fluxo apresentado. Antes de formalizar, confirme a condição no processo comercial oficial.",
      },
      {
        question: "É feita análise de crédito?",
        answer: "Para vendas na modalidade Tabela Investidor, a orientação exibida no simulador informa que não é feita análise de crédito. Ainda assim, confira a documentação indicada para pessoa física ou jurídica antes de formalizar a proposta.",
      },
    ],
  },
  {
    ...
    questions: [
      {
        ...
        answer: "Combine os filtros de Incorporadora, Empreendimento, Região, Planta e Valor do Imóvel. Depois, confira produto, metragem, data de entrega e valor antes de selecionar a unidade.",
      },
      {
        question: "Posso vender vaga de garagem avulsa nesta modalidade?",
        answer: "Não. Vagas de garagem avulsas são retiradas do estoque elegível da Tabela Investidor. Apartamentos que possuem vaga continuam disponíveis normalmente.",
      },
      {
        question: "Por que existem 8 propostas prontas?",
        answer: "São 4 combinações em 18 parcelas e 4 em 24 parcelas. Em cada prazo, você pode comparar propostas com ou sem sinais e com ou sem intermediárias antes de personalizar o fluxo.",
      },
      {
        question: "Quando devo usar uma proposta pronta?",
        answer: "Comece sempre pelos planos prontos. Eles ajudam a explicar as diferenças de entrada, pagamentos adicionais, saldo, valor das mensais e início do parcelamento. Personalize somente quando a condição do cliente exigir.",
      },
    ],
  },
  {
    ...
    questions: [
      {
        ...
        answer: "O ato deve representar pelo menos 6% do valor real. A entrada total, formada pelo ato mais os sinais aprovados, precisa atingir 10% para liberar o fluxo de até 18 parcelas e 20% para liberar até 24 parcelas, sempre limitada pelo prazo da obra.",
      },
      {
        question: "Como funcionam os sinais?",
        answer: "Podem ser usados até 3 sinais. Cada sinal ativo deve ser de pelo menos R$ 150,00. O Sinal 2 não pode ser maior que o Sinal 1, o Sinal 3 não pode ser maior que o Sinal 2 e não é permitido pular o sinal anterior.",
      },
      {
        question: "Como funcionam as intermediárias?",
        answer: "São permitidas até 3 intermediárias no fluxo de 18 parcelas e até 4 no fluxo de 24 parcelas. Cada uma pode representar no máximo 5% do valor real, deve coincidir com uma mensal e ocorrer até 3 meses antes do término da obra.",
      },
      {
        question: "Quando devo aplicar desconto?",
        answer: "Use o desconto somente quando solicitado pelo cliente e autorizado. Enquanto estiver desativado, ele não altera o cálculo da proposta.",
      },
    ],
  },
  {
    ...
    questions: [
      {
        ...
        answer: "O simulador trabalha com dias comerciais 5, 10 ou 15. Os sinais começam após a entrada e a primeira mensal é deslocada para depois do último sinal aprovado. As intermediárias seguem a cadência do plano e precisam coincidir com uma mensal.",
      },
      {
        question: "Por que o limite de parcelas pode ser menor que 18 ou 24?",
        answer: "18 e 24 são limites comerciais máximos. A quantidade realmente disponível também considera a entrada total, os sinais usados e o tempo restante até a data de entrega da obra.",
      },
      {
        question: "Como saber onde a proposta está errada?",
        answer: "Campos fora da regra recebem destaque e explicam o motivo do ajuste. Abra a Auditoria do cálculo para conferir entrada, sinais, intermediárias, prazo, saldo e parcela. Corrija todos os itens reprovados antes de apresentar a proposta.",
      },
      {
        question: "O que devo conferir no resultado?",
        answer: "Reconcilie valor do imóvel, entrada, sinais válidos, intermediárias válidas, saldo parcelado, quantidade de parcelas, valor mensal e datas inicial e final. O selo deve indicar que a proposta está dentro da regra.",
      },
    ],
  },
  {
    ...
    questions: [
      {
        ...
        answer: "No cabeçalho do resultado, use Doc Pessoa Física ou Doc Pessoa Jurídica. Abra a lista correta, confira todos os itens e valide eventuais dúvidas com o gerente ou a área responsável.",
      },
      {
        question: "Quando posso imprimir a proposta?",
        answer: "Imprima somente depois de confirmar a unidade, os valores, todas as datas, a documentação e a auditoria sem reprovações. Se a opção pronta estiver fechada, abra-a antes de imprimir para garantir que o plano desejado esteja visível.",
      },
      {
        question: "Qual é a melhor forma de apresentar as opções ao cliente?",
        answer: "Compare as alternativas disponíveis com o cliente e explique a diferença entre entrada imediata, sinais, intermediárias e mensais. Evite prometer condições que não estejam aprovadas no simulador e no fluxo oficial.",
      },
      {
        question: "O simulador substitui a validação oficial?",
        answer: "Não. Ele é uma ferramenta de apoio comercial. Confirme os dados da unidade, o resultado e a documentação no fluxo oficial antes de formalizar a proposta.",
      },
    ],
  },
] as const;
...
const ASSOCIATIVE_PROPOSAL_GUIDE_STEPS = [
  ...
  { title: "Digite o Financiamento", description: "Informe o valor aprovado pelo banco. Use um número maior que zero. Depois disso, o campo Subsídio será liberado.", note: "Digite somente o financiamento. Não some entrada, FGTS, subsídio ou Cheque Moradia neste campo." },
  { title: "Digite o Subsídio ou zero", description: "Se o cliente recebeu subsídio, informe o valor. Se não recebeu, digite zero. Assim, o campo FGTS será liberado.", note: "Nunca deixe o campo vazio: use zero para dizer ao sistema que não há subsídio." },
  { title: "Digite o FGTS ou zero", description: "Informe quanto do FGTS será usado na compra. Se o cliente não usar FGTS, digite zero. Depois, o Cheque Moradia será liberado.", note: "Use apenas o valor confirmado para esta proposta." },
  { title: "Digite o Cheque Moradia ou zero", description: "Informe o valor do benefício. Se não houver, digite zero. O sistema então libera a Entrada.", note: "Financiamento, Subsídio, FGTS e Cheque Moradia são descontados do valor do imóvel." },
  { title: "Informe a Entrada", description: "Digite quanto o cliente pagará na data mostrada ao lado do campo. A entrada mínima é R$ 150,00.", note: "Ajustes necessários nunca diminuem a entrada já digitada. Eles preservam esse valor e acrescentam somente o que faltar." },
  { title: "Escolha a quantidade de parcelas", description: "Digite um número inteiro entre 1 e o limite mostrado na tela. O sistema divide esse total entre o período antes e depois da entrega.", note: "Comece com 84 quando quiser a menor mensal possível. Se a quantidade não formar os 4 blocos, siga a mensagem de correção." },
  { title: "Selecione o Ranking", description: "Abra a lista à direita e escolha a classificação correta. O sistema compara os fluxos Linear e Decrescente com os limites desse Ranking.", note: "Sem Ranking, sinais, anuais, desconto e resultado de aprovação continuam bloqueados." },
  { title: "Adicione Sinais somente se precisar", description: "Use Inserir Sinal para criar até 3 pagamentos extras. Preencha na ordem: Sinal 1, depois 2 e depois 3.", note: "Cada sinal precisa ter pelo menos R$ 150,00. Um ajuste automático preserva os sinais já informados e só aumenta o necessário." },
  { title: "Adicione Anuais somente se precisar", description: "Use Inserir Anual para incluir pagamentos em 15 de dezembro, enquanto a obra estiver em andamento. Elas reduzem a base das mensais, mas nunca o Pró-Soluto.", note: "Cada anual pode chegar a 50% da renda familiar. O sistema preserva os valores já digitados e aplica a correção mostrada na ajuda." },
  { title: "Leia o resultado e os ajustes", description: "APROVADO significa que o fluxo está dentro do Ranking. REPROVADO mostra o botão Ver ajustes necessários, com uma prévia antes de qualquer mudança.", note: "Confira a prévia. Só clique em Aplicar estes valores se quiser substituir a proposta pelos valores mostrados." },
  { title: "Confira parcelas, documentos e impressão", description: "Abra Exibir parcelas para revisar valores e datas. Depois confira a documentação e imprima somente a proposta final.", note: "Antes de apresentar ao cliente, confirme unidade, renda, recursos, entrada, Ranking, datas e status de aprovação." },
] as const;
...
const DIRECT_TABLE_FAQ = [
  ...
  { question: "O cliente pode migrar o saldo para um banco?", answer: "A política permite optar por financiamento bancário após seis meses do Habite-se. As condições precisam ser confirmadas no fluxo oficial antes de qualquer compromisso com o cliente." },
  { question: "Quando o registro pode começar?", answer: "Após a averbação do Habite-se e a disponibilidade dos documentos do imóvel. O cliente deve estar adimplente e pode conduzir o processo ou contratar despachante; o prazo médio informado é de 50 dias." },
] as const;

function DocumentationDialog({ type, dialogRef, directTable = false, associative = false }: { type: keyof typeof PERSON_DOCUMENTATION; dialogRef: Ref<HTMLDialogElement>; directTable?: boolean; associative?: boolean }) {
  const content = associative && type === "pf" ? ASSOCIATIVE_PERSON_DOCUMENTATION : directTable ? DIRECT_PERSON_DOCUMENTATION[type] : PERSON_DOCUMENTATION[type];
  ...
  return <dialog ref={dialogRef} id={`investor-documentation-${type}`} className="investor-documentation-dialog" aria-labelledby={titleId} onClick={(event) => { if (event.target === event.currentTarget) event.currentTarget.close(); }}>
    <article>
      <header><h2 id={titleId}>{content.title}</h2><form method="dialog"><button type="submit" aria-label={`Fechar ${content.title}`}>×</button></form></header>
      {directTable || (associative && type === "pf") ? <div className="investor-documentation-sections">
        {(associative && type === "pf" ? ASSOCIATIVE_PERSON_DOCUMENTATION.sections : directContent.sections).map((section, sectionIndex) => <section key={section.title} aria-labelledby={`investor-documentation-${type}-section-${sectionIndex}`}>
          <h3 id={`investor-documentation-${type}-section-${sectionIndex}`}><span aria-hidden="true">{String(sectionIndex + 1).padStart(2, "0")}</span>{section.title}</h3>
          <ol>{section.items.map((item) => <li key={item}>{item}</li>)}</ol>
        </section>)}
        <p className="investor-documentation-note"><strong>Observação</strong><span>{associative && type === "pf" ? ASSOCIATIVE_PERSON_DOCUMENTATION.note : directContent.note}</span></p>
      </div> : <>
        <ol>{standardContent.items.map((item) => <li key={item}>{item}</li>)}</ol>
        <p><em>Obs.: União estável não substitui a Certidão de estado Civil.</em></p>
      </>}
    </article>
  </dialog>;
}

function DirectAmortizationDialog({ dialogRef, principal, schedule }: { dialogRef: Ref<HTMLDialogElement>; principal: number; schedule: ReturnType<typeof buildDirectTableAmortizationSchedule> }) {
  return <dialog ref={dialogRef} id="investor-direct-amortization" className="investor-documentation-dialog investor-direct-amortization-dialog" aria-labelledby="investor-direct-amortization-title" onClick={(event) => { if (event.target === event.currentTarget) event.currentTarget.close(); }}>
    <article>
      <header>
        <div><span>Memória de cálculo</span><h2 id="investor-direct-amortization-title">{schedule.length} parcelas pós-chaves</h2><p>Saldo inicial {money.format(principal)} · juros de 12% a.a. · seguros MIP e DFI.</p></div>
        <form method="dialog"><button type="submit" aria-label="Fechar tabela de amortização">×</button></form>
      </header>
      <div className="investor-direct-amortization-scroll" role="region" tabIndex={0} aria-label="Tabela de amortização das parcelas pós-chaves">
        <table>
          <thead><tr><th scope="col">Mês</th><th scope="col">Amortização</th><th scope="col">Juros</th><th scope="col">Seguro MIP</th><th scope="col">Seguro DFI</th><th scope="col">Parcela total</th><th scope="col">Data da parcela</th><th scope="col">Saldo devedor</th></tr></thead>
          <tbody>
            <tr><th scope="row">0</th><td>—</td><td>—</td><td>—</td><td>—</td><td>—</td><td>—</td><td>{money.format(principal)}</td></tr>
            {schedule.map((item) => <tr key={item.month}><th scope="row">{item.month}</th><td>{money.format(item.amortization)}</td><td>{money.format(item.interest)}</td><td>{money.format(item.mip)}</td><td>{money.format(item.dfi)}</td><td><strong>{money.format(item.totalPayment)}</strong></td><td>{formatDate(item.paymentDate)}</td><td>{money.format(item.balance)}</td></tr>)}
          </tbody>
        </table>
      </div>
    </article>
...
function DirectPreKeysDialog({ dialogRef, principal, schedule }: { dialogRef: Ref<HTMLDialogElement>; principal: number; schedule: ReturnType<typeof buildDirectTablePreKeysSchedule> }) {
  return <dialog ref={dialogRef} id="investor-direct-pre-keys" className="investor-documentation-dialog investor-direct-amortization-dialog" aria-labelledby="investor-direct-pre-keys-title" onClick={(event) => { if (event.target === event.currentTarget) event.currentTarget.close(); }}>
    <article>
      <header>
        <div><span>Memória de cálculo</span><h2 id="investor-direct-pre-keys-title">{schedule.length} parcelas pré-chaves</h2><p>Saldo inicial {money.format(principal)} · parcelas mensais sem juros e seguros.</p></div>
        <form method="dialog"><button type="submit" aria-label="Fechar tabela das parcelas pré-chaves">×</button></form>
      </header>
      <div className="investor-direct-amortization-scroll investor-direct-pre-keys-scroll" role="region" tabIndex={0} aria-label="Tabela das parcelas mensais pré-chaves">
        <table>
          <thead><tr><th scope="col">Mês</th><th scope="col">Parcela</th><th scope="col">Data da parcela</th><th scope="col">Saldo devedor</th></tr></thead>
          <tbody>
            <tr><th scope="row">0</th><td>—</td><td>—</td><td>{money.format(principal)}</td></tr>
            {schedule.map((item) => <tr key={item.month}><th scope="row">{item.month}</th><td><strong>{money.format(item.payment)}</strong></td><td>{formatDate(item.paymentDate)}</td><td>{money.format(item.balance)}</td></tr>)}
          </tbody>
        </table>
      </div>
    </article>
...
function AssociativeInstallmentDialog({
  ...
  const dialogTitle = viewMode === "comparison" ? "Linear × Decrescente" : viewMode === "linear" ? "Tabela Linear" : "Tabela Decrescente";

  return <dialog ref={dialogRef} id="investor-associative-installments" className="investor-documentation-dialog investor-direct-amortization-dialog investor-associative-installment-dialog" aria-labelledby="investor-associative-installments-title" onClick={(event) => { if (event.target === event.currentTarget) event.currentTarget.close(); }}>
    <article>
      <header>
        <div className="investor-associative-dialog-heading"><span>Todos os pagamentos</span><h2 id="investor-associative-installments-title">{dialogTitle}</h2></div>
        ...
        <nav className="investor-associative-comparison-tabs" aria-label="Exibição e impressão das parcelas">
          ...
          <button type="button" className="investor-associative-print-button" onClick={() => window.print()} aria-label="Imprimir valores atuais da tabela">Imprimir</button>
        </nav>
        <form method="dialog"><button type="submit" aria-label="Fechar comparativo das parcelas associativas">×</button></form>
      </header>
      <p className="investor-associative-comparison-formula">% Obra parte de {normalizedProgress == null ? "um valor ainda não informado" : percent.format(normalizedProgress)} e chega a 100% na entrega. Evolução = renda × 30% × % Obra; mês vigente e próximo mês formam a carência de assinatura.</p>
      <div className={`investor-associative-installment-scroll mode-${viewMode}`} role="region" aria-label="Todos os pagamentos e comparativo das parcelas lineares e decrescentes" tabIndex={0}>
        <table>
          <thead>
            <tr className="investor-associative-comparison-groups"><th rowSpan={2} scope="col">Nº</th><th rowSpan={2} scope="col">Pagamento</th><th className="is-work-group" colSpan={2} scope="colgroup">Obra</th>{hasAnnuals ? <th className="is-annual-column" rowSpan={2} scope="col">Anual</th> : null}{showLinear ? <th className="is-linear-group" colSpan={3} scope="colgroup">Fluxo Linear</th> : null}{showDecreasing ? <th className="is-decreasing-group" colSpan={3} scope="colgroup">Fluxo Decrescente</th> : null}<th rowSpan={2} scope="col">Data Parcela</th></tr>
            <tr><th className="is-work-column" scope="col">% Obra</th><th className="is-work-column" scope="col">Evolução Obra</th>{showLinear ? <><th className="is-linear-column" scope="col">Parcela Linear</th><th className="is-linear-column" scope="col">Total Linear</th><th className="is-linear-column" scope="col">% da Renda</th></> : null}{showDecreasing ? <><th className="is-decreasing-column" scope="col">Decrescente</th><th className="is-decreasing-column" scope="col">Total Decrescente</th><th className="is-decreasing-column" scope="col">% da Renda</th></> : null}</tr>
          </thead>
          <tbody>
            {comparisonRows.map((item, index) => <tr key={`${item.kind}-${item.installment ?? item.label}-${item.paymentDate}-${index}`} className={`is-${item.kind}`}>
              <td data-label="Nº">{item.installment ?? "—"}</td>
              <td data-label="Pagamento"><strong>{item.label}</strong></td>
...
export function InvestorLearningManual({ directTable = false, associative = false }: { directTable?: boolean; associative?: boolean }) {
      ...
      <article>
        <header>
          <div>
            ...
            <h2 id="investor-learning-title">{directTable ? "Aprenda Tabela Direta" : `Aprenda ${modality}`}</h2>
          </div>
          <form method="dialog"><button type="submit" aria-label={`Fechar manual da ${modality}`}>×</button></form>
        </header>
        <p id="investor-learning-description" className="investor-learning-intro">Consulte a política completa e as respostas objetivas para conduzir a simulação com segurança.</p>
        {directTable ? <div className="investor-direct-learning-content">
          ...
          <section id="investor-direct-learning-policy" className="investor-direct-learning-policy" aria-labelledby="investor-direct-learning-policy-title">
            <div className="investor-direct-learning-section-heading"><span aria-hidden="true">01</span><h3 id="investor-direct-learning-policy-title">Política</h3></div>
            <div>
              {DIRECT_TABLE_POLICY_TOPICS.map((topic) => <section key={topic.title}>
                <h4>{topic.title}</h4>
                <ul>{topic.items.map((item) => <li key={item}>{item}</li>)}</ul>
              </section>)}
            </div>
          </section>
          <section id="investor-direct-learning-faq" className="investor-direct-learning-faq" aria-labelledby="investor-direct-learning-faq-title">
            <div className="investor-direct-learning-section-heading"><span aria-hidden="true">02</span><h3 id="investor-direct-learning-faq-title">Perguntas</h3></div>
            <div>
              {DIRECT_TABLE_FAQ.map((item) => <details key={item.question}>
          ...
          <ul className="investor-learning-highlights" aria-label="Resumo do manual">
            <li><strong>{associative ? "2" : "8"}</strong><span>{associative ? "fluxos comparados" : "propostas prontas"}</span></li>
            <li><strong>3</strong><span>sinais no máximo</span></li>
            <li><strong>{associative ? "Até 84" : "18 ou 24"}</strong><span>parcelas máximas</span></li>
            <li><strong>{associative ? "PF" : "PF e PJ"}</strong><span>{associative ? "documentação do cliente" : "listas de documentos"}</span></li>
          </ul>
          <div className="investor-learning-sections">
            {(associative ? ASSOCIATIVE_MANUAL_SECTIONS : INVESTOR_MANUAL_SECTIONS).map((section) => <section key={section.title}>
              <h3>{section.title}</h3>
              <div>
                {section.questions.map((item) => <details key={item.question}>
                  <summary>{item.question}</summary>
          ...
          <aside className="investor-learning-checklist">
            ...
            <ul>
              <li>Unidade, valor e data de entrega confirmados.</li>
              <li>Entrada, sinais, {associative ? "anuais" : "intermediárias"} e mensais reconciliados.</li>
              <li>{associative ? "Parâmetros de aprovação e memória comparativa conferidos." : "Auditoria sem reprovações e documentação conferida."}</li>
              <li>Resultado validado no fluxo comercial oficial.</li>
            </ul>
          </aside>
        </>}
        <footer><form method="dialog"><button type="submit">Fechar manual</button></form></footer>
      </article>
    </dialog>
  </section>;
}
...
function PropertySummary({ item, label, associative = false }: { item: InventoryItem; label: string; associative?: boolean }) {
  return <article className={`investor-selected-unit investor-property-summary${associative ? " is-associative" : ""}`} aria-label={label} data-tour="property-summary">
    ...
    <dl>
      <div tabIndex={0}><dt>Incorporadora</dt><dd>{informationLabel(item.businessUnit)}</dd></div>
      <div tabIndex={0}><dt>Planta</dt><dd>{informationLabel(item.plant)}</dd></div>
      ...
      {associative ? <>
        ...
        <div tabIndex={0}><dt>Outras Descrições</dt><dd>{informationLabel(item.classification)}</dd></div>
      </> : null}
    </dl>
  </article>;
}
...
function compactProductDescription(product: string, project: string) {
  ...
  return trimmedProduct.toLocaleLowerCase("pt-BR").endsWith(repeatedSuffix.toLocaleLowerCase("pt-BR"))
...
function inferLocation(identifier: string | null) {
  ...
  if (digits.length === 3) return { floor: Number(digits.slice(0, 1)), finalUnit: Number(digits.slice(1)) };
...
function enrichInventory(items: InventoryItem[], reference: InventoryItem[]) {
  ...
  return items.map((item) => {
    ...
    const finalPrice = finalWithKit != null
    ...
    return {
      ...
      neighborhood: item.neighborhood ?? source?.neighborhood ?? projectSource?.neighborhood ?? null,
...
export function InvestorCalculator({
  ...
  const [selectedScenarioPlan, setSelectedScenarioPlan] = useState<number>(18);
  ...
  const associativeFinancingDecision = useMemo(() => evaluateFinancingModality({
  ...
  const associativeEntryUnlocked = associativeHousingCheckUnlocked && associativeHousingCheckComplete;
  const associativeQualificationComplete = !annualMode || (
  ...
  useEffect(() => {
    ...
    const changedDimension = (["businessUnit", "project", "plant", "region", "salePrice"] as const)
  ...
  const associativeInstallmentSchedule = useMemo(() => annualMode && result.custom.linear
    ...
    : [], [annualMode, result.context.monthlyDates, result.custom.linear, result.custom.postInstallments, result.custom.preInstallments]);
  const associativePaymentComparison = useMemo(() => buildAssociativePaymentComparison({
    monthlyDates: result.context.monthlyDates,
    installments: result.custom.desiredInstallments,
  ...
  const associativeAnnualIncomeFailure = annualMode && associativeIncomeValue > 0
    ? result.custom.intermediaries.find((item: { value: number }) => item.value > associativeAnnualIncomeLimit)
    : undefined;
  const associativeBlockCounts = annualMode
    ? result.custom.decreasing?.blocks?.map((block: { count: number }) => block.count) ?? []
    : [];
  const associativeBlockDistributionError = annualMode
    && result.custom.desiredInstallments > 0
    && (
      associativeBlockCounts.length !== 4
      || associativeBlockCounts.some((count: number) => count <= 0)
      || associativeBlockCounts.reduce((total: number, count: number) => total + count, 0) !== result.custom.desiredInstallments
    )
    ? "A quantidade informada precisa formar os quatro blocos com pelo menos uma parcela em cada."
    : "";
  const associativeInstallmentsRejected = annualMode && (
  ...
  const associativeEntryReady = annualMode
    && associativeEntryUnlocked
    && !associativeEntryPending
    && !associativeEntryRejected;
  const associativeInstallmentsUnlocked = annualMode
    && associativeFinancingValueReady
    && associativeEntryReady;
  const associativeInstallmentsReady = associativeInstallmentsUnlocked
  ...
  const associativeSignalFailure = annualMode
    ? result.custom.signals.find((item: { active: boolean; approved: boolean }) => item.active && !item.approved)
    : undefined;
      ...
      : associativeSignalFailure
        ...
        : associativeAnnualRuleFailure
          ...
          : associativeAnnualIncomeFailure
            ...
            : associativeInstallmentsRejected
              ? associativeBlockDistributionError || `Qtd. de parcelas: use um número inteiro entre 1 e ${result.context.maxInstallments}.`
              : !result.ok
                ? result.errors?.[0] || "Revise os campos destacados no Fluxo editável."
                : undefined;
  const activeSignalFieldCount = signals.reduce((latest, value, index) => currencyInputNumber(value) > 0 ? index + 1 : latest, 0);
  const signalVisibilityFloor = Math.max(signalFieldCount, activeSignalFieldCount, signalsRequired ? 1 : 0);
  ...
  const firstInstallmentDate = validInstallmentSchedule ? result.context.monthlyDates[0] ?? "" : "";
  const lastInstallmentDate = validInstallmentSchedule ? result.context.monthlyDates[result.context.monthlyDates.length - 1] ?? "" : "";
  const associativeLinearUncorrected = annualMode && result.custom.desiredInstallments > 0
  ...
  const directIntermediaryPayments: DirectCalculationPayment[] = selectedDirectProposalOption?.withIntermediary
    ? result.custom.intermediaries
      ...
      .map((item: { index: number; value: number; date: string; reason: string; approved: boolean }) => ({
        key: `intermediary-${item.index}`,
        label: `Intermediária ${item.index}`,
        operator: item.approved ? "−" : "!",
        calculation: item.approved ? `${money.format(result.context.valueReal)} × ${percent.format(item.value / result.context.valueReal)}` : "Regra de 5% não aplicada",
  ...
  const directReadyPayments: DirectCalculationPayment[] = selectedDirectProposalOption ? [
    { key: "property", label: "Valor real da venda", operator: "=", calculation: "", result: money.format(result.context.valueReal), amount: result.context.valueReal, meta: "Valor real usado como base para montar esta proposta." },
    { key: "entry", label: "Ato", operator: "−", calculation: `${money.format(result.context.valueReal)} × ${percent.format(result.custom.actRate)}`, result: `− ${money.format(result.custom.actValue)}`, amount: result.custom.actValue, meta: `Pagamento em ${formatDate(baseDate)}` },
    { key: "balance-after-entry", label: "Saldo após o ato", operator: "=", calculation: `${money.format(result.context.valueReal)} − ${money.format(result.custom.actValue)}`, result: money.format(result.context.valueReal - result.custom.actValue), amount: result.context.valueReal - result.custom.actValue, meta: "Valor restante após o pagamento inicial" },
    ...result.custom.signals
      .filter((signal: { active: boolean }) => signal.active)
      .map((signal: { index: number; value: number; date: string; approved: boolean; reason: string }) => ({
        key: `signal-${signal.index}`,
        label: `Sinal ${signal.index}`,
        operator: signal.approved ? "−" : "!",
        calculation: signal.approved ? `${money.format(result.context.valueReal)} × ${percent.format(signal.value / result.context.valueReal)}` : "Regra do sinal não aplicada",
  ...
  useEffect(() => {
    ...
    const currentTarget = document.querySelector<HTMLElement>(`[data-tour="${tourSteps[tourStep].target}"]`);
    ...
    const closeOnEscape = (event: KeyboardEvent) => {
      const popupControl = event.target instanceof Element ? event.target.closest("select,[aria-haspopup]") : null;
    ...
    currentTarget.scrollIntoView({ behavior: reduceMotion ? "auto" : "smooth", block: "center", inline: "nearest" });
  ...
  function guideToSection(target: "qualification" | "flow") {
    ...
    const section = target === "qualification" ? associativeQualificationSectionRef.current : associativeFlowSectionRef.current;
    if (!section) return;
    const reduceMotion = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
    if (guidedAttentionTimer.current !== null) window.clearTimeout(guidedAttentionTimer.current);
  ...
  function focusNextAssociativeRow(event: ReactKeyboardEvent<HTMLOListElement>) {
    ...
    window.requestAnimationFrame(() => {
      const nextInput = Array.from(ledger.children)
        ...
        .map((row) => row.querySelector<HTMLInputElement>('input:not([disabled]):not([type="hidden"])'))
      ...
      ledger
        ...
        ?.querySelector<HTMLSelectElement>('select[name="classificacao-associativo"]:not([disabled])')
        ?.focus();
    });
  }
  ...
  function selectScenarioPlan(plan: number) {
    setSelectedScenarioPlan(plan);
    setVisibleScenarioCodes((current) => current.filter((code) => scenarioOptions[code]?.plan === plan));
  }
  ...
  function buildAssociativeFlowSuggestion(flow: AssociativeFlow): AssociativeFlowSuggestion | null {
    ...
    const annualEligible = result.custom.intermediaries.map((item: { date: string }) => Boolean(
      item.date && baseDate && completionDate && item.date >= baseDate && item.date <= completionDate,
    ));
  ...
  function applyAssociativeFlowSuggestion(_flow: AssociativeFlow, suggestion: AssociativeFlowSuggestion) {
    setInstallments(String(suggestion.installments));
    setIntermediaries(suggestion.annuals.map((value) => value > 0 ? value.toFixed(2) : "0"));
    setIntermediaryFieldCount(suggestion.annuals.reduce((latest, value, index) => value > 0 ? index + 1 : latest, 0));
    setHiddenIntermediaryIndexes([]);
    ...
    return (
        <section className={`investor-standard-panel ${directTable ? "investor-direct-combined-panel" : directVisualLayout ? "investor-associative-direct-panel" : ""}${associativeCalculatedProposalLocked ? " is-locked" : ""}${annualMode && associativeQualificationComplete && !associativeFinancingValueReady ? " is-awaiting-financing" : ""}`} aria-labelledby={directTable ? "investor-direct-income-title investor-standard-title" : "investor-standard-title"} data-locked={associativeCalculatedProposalLocked || undefined} data-tour="scenarios">
          {!directTable ? <header className="investor-section-heading investor-standard-heading">
            ...
            <div><p>{annualMode ? "Proposta calculada" : "Escolha rápida"}</p><h2 id="investor-standard-title">{directVisualLayout ? "1 plano · 4 opções" : "8 opções prontas disponíveis"}</h2></div>
            <div className="investor-standard-actions">
              <InvestorInfoHint
                label="propostas prontas"
                title="Como funcionam as propostas prontas?"
                description={directVisualLayout
              ...
              <button
                ...
                aria-label={visibleScenarioCodes.length === 0 ? "Abra uma opção para imprimir" : "Imprimir as opções abertas"}
                title={visibleScenarioCodes.length === 0 ? "Abra pelo menos uma opção para imprimir" : "Imprimir as opções abertas"}
              >Imprimir</button>
            </div>
          </header> : null}
          {associativeCalculatedProposalLocked ? <div className={`investor-associative-section-lock${associativeQualificationComplete ? " awaiting-financing" : ""}`} role="status" aria-live="polite"><span aria-hidden="true">{associativeQualificationComplete ? "PRÓXIMA AÇÃO" : "BLOQUEADO"}</span><div><strong>Proposta calculada bloqueada</strong><small>{associativeProposalMissingLabel} Renda, financiamento, recursos e quantidade de parcelas precisam estar confirmados.</small></div><button type="button" onClick={() => associativeQualificationComplete ? associativeFinancingInputRef.current?.focus() : associativeIncomeInputRef.current?.focus()}>{associativeQualificationComplete ? "Ir ao fluxo" : "Responder perguntas"}</button></div> : null}
          <fieldset className="investor-associative-gated-fieldset" disabled={associativeCalculatedProposalLocked} aria-disabled={associativeCalculatedProposalLocked || undefined}>
          ...
          {directTable ? <>
            <div className="investor-direct-five-card-headings">
              <header className="investor-section-heading investor-standard-heading investor-direct-income-heading">
                <span>02</span>
                <div><p id="investor-direct-income-title">Informe a renda</p></div>
              </header>
              <header className="investor-section-heading investor-standard-heading investor-direct-choice-heading">
                <span>03</span>
                <div><p>Escolha rápida</p><h2 id="investor-standard-title">4 opções prontas disponíveis</h2></div>
              </header>
            </div>
            <div className="investor-direct-five-card-grid" role="group" aria-label="Renda e opções de proposta da Tabela Direta">
              <label className="investor-installments-field investor-direct-income-card" data-tour="income">
                <span className="investor-installments-heading"><span>Renda mensal</span></span>
                <MoneyInput inputRef={directIncomeInputRef} label="Renda mensal" describedBy="investor-income-guidance" value={income} onChange={updateIncome} />
              </label>
                        ...
                        {directProposalComparison.length > 0 ? <div className="investor-direct-comparison-grid is-single" role="region" aria-label="Composição atual da proposta">
                          ...
                          <ol>{directCalculationSteps.map((payment, index) => {
                            ...
                            return <DirectEditableAccountRow
                              ...
                              label={payment.label}
                              meta={payment.meta}
                              calculation={<DirectReadyLedgerValue label={payment.label} amount={payment.amount} text={payment.result} help={help} action={action} />}
            ...
            {directVisualLayout ? <div className="investor-direct-five-card-grid investor-associative-direct-grid" role="group" aria-label="Opções prontas da Tabela Associativo">
              <div className="investor-direct-ready-options">
                {result.standardScenarios.map((scenario, index) => {
                  ...
                  const availabilityMessage = scenario.availabilityReason ?? "Cenário fora da regra";
                  return <div key={scenario.code} className="investor-scenario-plan" data-scenario={option.plan === 18 ? 1 : 2}>
                    <button
                      type="button"
                    ...
                    >
                      <span className="investor-direct-option-number" aria-hidden="true">{String(index + 1).padStart(2, "0")}</span>
                      <strong>{option.title}</strong>
                      <span className="investor-direct-option-entry">Plano em {scenario.installments} parcelas</span>
                      <small id={!scenario.available ? availabilityId : undefined}>{scenario.available ? option.description : availabilityMessage}</small>
                      {visible ? <span className="investor-direct-option-selected">Selecionada</span> : null}
                    </button>
                  </div>;
                })}
              </div>
            </div> : <div className="investor-standard-quick-picker">
              <div className="investor-standard-plan-switch" role="group" aria-label="Prazo das propostas prontas">
                {scenarioPlans.map((plan) => <button
                  key={plan}
                  id={`investor-scenario-plan-${plan}-button`}
                  type="button"
                  aria-pressed={selectedScenarioPlan === plan}
                  aria-controls="investor-standard-ready-options"
                  onClick={() => selectScenarioPlan(plan)}
                >Plano em <strong>{plan} parcelas</strong></button>)}
              </div>
              <div id="investor-standard-ready-options" className="investor-direct-ready-options investor-standard-ready-options" role="group" aria-label={`Opções prontas do plano em ${selectedScenarioPlan} parcelas`}>
                {result.standardScenarios.filter((scenario) => scenarioOptions[scenario.code]?.plan === selectedScenarioPlan).map((scenario, index) => {
                  const option = scenarioOptions[scenario.code];
                  const visible = visibleScenarioCodes.includes(scenario.code);
                  const availabilityId = `investor-scenario-availability-${scenario.code}`;
                  const availabilityMessage = `O prazo da obra não comporta ${option.plan} parcelas completas`;
                  return <div key={scenario.code} className="investor-scenario-plan" data-scenario="1">
                    <button
                      type="button"
                      aria-pressed={visible}
                      aria-describedby={!scenario.available ? availabilityId : undefined}
                    ...
                    >
                      <span className="investor-direct-option-number" aria-hidden="true">{String(index + 1).padStart(2, "0")}</span>
                      <strong>{option.title}</strong>
                      <span className="investor-direct-option-entry">Entrada de {percent.format(scenario.entryRate)}</span>
                      <small id={!scenario.available ? availabilityId : undefined}>{scenario.available
                        ? <>{scenario.signals.length > 0 ? `${scenario.signals.length} sinais somam ${percent.format(scenario.signalTotal / result.context.valueReal)}` : "Sem sinais"}<br />{scenario.intermediaryCount > 0 ? `${scenario.intermediaryCount} intermediárias somam ${percent.format(scenario.intermediaryTotal / result.context.valueReal)}` : "Sem intermediária"}</>
                        : "Prazo da obra insuficiente"}</small>
                      {visible ? <span className="investor-direct-option-selected">Selecionada</span> : null}
                    </button>
                  </div>;
                })}
            ...
            <div className="investor-scenario-stack" aria-live="polite">
              {visibleScenarioCodes.map((code) => {
                const scenario = result.standardScenarios.find((scenario) => scenario.code === code);
                ...
                const verticalPayments = linear ? [
                  { key: "property", label: "Valor real do imóvel", operator: "", calculation: "Preço do imóvel − deduções comerciais", result: money.format(linear.realSaleValue), meta: "Base selecionada no estoque" },
                  { key: "financing", label: "Financiamento", operator: "−", calculation: "Recurso bancário aplicado nesta proposta", result: `− ${money.format(scenario.financing)}`, meta: "Valor ajustado para preservar a aprovação" },
                  ...
                  { key: "corrected-pro-soluto", label: "Pró-soluto corrigido", operator: "+", calculation: `${money.format(linear.proSoluto)} corrigido pela carência`, result: money.format(linear.correctedProSoluto), meta: `Taxa-base de ${percent.format(linear.baseRate)} a.m.` },
                ...
                if (directVisualLayout) {
                  ...
                  return <section id={`investor-scenario-option-${scenario.code}`} key={scenario.code} className="investor-scenario-group investor-scenario-option-panel investor-direct-option-panel investor-associative-option-panel" data-scenario={scenario.signals.length > 0 ? 2 : 1}>
                    <header><div /><button type="button" onClick={() => toggleScenarioOption(scenario.code)} aria-label={`Ocultar ${optionTitle}`} title="Ocultar proposta"><span aria-hidden="true">×</span></button></header>
                    <div className="investor-scenario-grid"><article>
                      <section className="investor-direct-calculation" aria-labelledby={calculationTitleId}>
                        <header className="investor-direct-calculation-heading">
                          <div><span>Proposta pronta</span><h4 id={calculationTitleId}>{`Composição do pagamento ${option.title.replace(/^Proposta /u, "").toLocaleLowerCase("pt-BR")}`}</h4><p>{`Plano em ${scenario.installments} parcelas · ${optionDescription}`}</p></div>
                          <div className={`investor-direct-credit-status ${scenario.available ? "approved" : "pending"}`}><small>Resultado da proposta</small><strong>{scenario.available ? "VALIDADO" : "PENDENTE"}</strong><span>{`${scenario.preInstallments} pré + ${scenario.postInstallments} pós = ${scenario.installments}`}</span></div>
                        </header>
                        <div className="investor-direct-calculation-body">
                          <div className="investor-direct-account" role="region" tabIndex={0} aria-label={`${optionTitle}: memória de cálculo`}>
                            <div className="investor-direct-account-legend" aria-hidden="true"><span>Etapa</span><span>Cálculo</span><span>Resultado</span></div>
                            <ol>{verticalPayments.map((payment, index) => <li key={payment.key} className={payment.featured ? "total" : ""}>
                              <span className="investor-direct-step-number" aria-hidden="true">{String(index + 1).padStart(2, "0")}</span>
                              <div className="investor-direct-step-content">
                                <div className="investor-direct-step-name"><span aria-hidden="true">{payment.operator}</span><div><strong>{payment.label}</strong><small>{payment.meta}</small></div></div>
                                <div className="investor-direct-step-formula">{payment.calculation}</div>
                                <div className="investor-direct-step-result"><strong>{payment.result}</strong></div>
                              </div>
                            </li>)}</ol>
                          </div>
                        </div>
                ...
                return <section id={`investor-scenario-option-${scenario.code}`} key={scenario.code} className="investor-scenario-group investor-scenario-option-panel investor-direct-option-panel investor-standard-option-panel" data-scenario="1">
                  <div className="investor-scenario-grid">
                    <article>
                      <div className="investor-direct-comparison-grid is-single" role="region" aria-label={`${optionTitle}: resultado calculado`}>
                        <InvestorScenarioComparisonCard scenario={scenario} option={option} optionNumber={result.standardScenarios.filter((candidate) => scenarioOptions[candidate.code]?.plan === option.plan).findIndex((candidate) => candidate.code === scenario.code) + 1} valueReal={result.context.valueReal} baseDate={baseDate} />
                      </div>
                    </article>
                  </div>
                </section>;
            ...
            <p className="investor-scenario-order">{annualMode ? "Compare as opções liberadas após o financiamento informado no fluxo acima." : "Selecione uma opção pronta por vez e, se necessário, ajuste a composição completa no fluxo editável abaixo."}</p>
          </>}
          </fieldset>
        </section>
    ...
    <div className={`investor-workspace${usesDirectDesign ? " investor-direct-workspace" : ""}${directVisualLayout ? " investor-direct-design-copy" : ""}${directTable ? directPrintReady ? " investor-direct-print-ready" : " investor-direct-print-blocked" : ""}`}>
      ...
      {tourOpen ? <>
        <div
          ...
          aria-hidden="true"
          style={{ top: tourSpotlight.top, left: tourSpotlight.left, width: tourSpotlight.width, height: tourSpotlight.height }}
        />
        <aside
        ...
        >
          <header>
            <span>Passo {tourStep + 1} de {tourSteps.length}</span>
            <button type="button" onClick={closeGuidedTour} aria-label="Fechar passo a passo">×</button>
          </header>
          <div className="investor-tour-progress" role="progressbar" aria-label="Progresso do guia" aria-valuemin={1} aria-valuemax={tourSteps.length} aria-valuenow={tourStep + 1}>
          ...
          <div className="investor-tour-copy" aria-live="polite">
            ...
            {directTable && tourSteps[tourStep].target === "information" ? <div id="investor-guided-tour-description" className="investor-tour-info-demo"><span className="investor-info-mark" aria-hidden="true" /><p>{tourSteps[tourStep].description}</p></div> : <>
              <p id="investor-guided-tour-description">{tourSteps[tourStep].description}</p>
              {tourSteps[tourStep].checklist.length ? <ul>{tourSteps[tourStep].checklist.map((item) => <li key={item}>{item}</li>)}</ul> : null}
              {tourSteps[tourStep].tip ? <small>{tourSteps[tourStep].tip}</small> : null}
            </>}
          </div>
          {tourSteps[tourStep].target === "inventory" && !selectedUnit ? <p className="investor-tour-action" role="status">Selecione uma unidade para liberar o próximo passo.</p> : null}
          <footer>
            <button type="button" className="secondary" onClick={showPreviousTourStep} disabled={tourStep === 0}>Anterior</button>
            <button type="button" onClick={showNextTourStep} disabled={tourSteps[tourStep].target === "inventory" && !selectedUnit}>
              {tourStep === tourSteps.length - 1 ? "Concluir guia" : tourSteps[tourStep].target === "inventory" && !selectedUnit ? "Selecione uma unidade" : "Próximo"}
            </button>
          </footer>
        </aside>
      </> : null}
      ...
      {selectedUnit ? (
        <div className="investor-proposal-layout">
          <section ref={annualMode ? associativeFlowSectionRef : undefined} tabIndex={annualMode ? -1 : undefined} className={`investor-flow-panel${usesDirectDesign ? " investor-direct-flow-panel" : ""}${annualMode ? " investor-associative-flow-panel" : ""}${associativeQualificationLocked ? " is-locked" : ""}${guidedAttention === "flow" ? " is-guided-active" : ""}`} aria-labelledby="investor-flow-title" data-locked={associativeQualificationLocked || undefined} data-tour="proposal">
                ...
                <div className="investor-proposal-help-action">
                  ...
                  <div className="investor-proposal-help-cta">
                    ...
                    <button
                      ...
                      aria-haspopup="dialog"
                      aria-controls={directTable ? "investor-guided-tour" : "investor-proposal-guide"}
                      onClick={directTable ? startProposalGuidedTour : () => { setTourOpen(false); proposalGuideDialog.current?.showModal(); }}
                    >Iniciar passo a passo</button>
                  </div>
                </div>
              </aside>
            ...
            {associativeQualificationLocked ? <div className="investor-associative-section-lock" role="status" aria-live="polite"><span aria-hidden="true">BLOQUEADO</span><div><strong>Fluxo editável bloqueado</strong><small>Responda Renda Familiar, Modalidade e Primeiro imóvel.</small></div><button type="button" onClick={() => associativeIncomeInputRef.current?.focus()}>Responder perguntas</button></div> : null}
            <fieldset className="investor-associative-gated-fieldset" disabled={associativeQualificationLocked} aria-disabled={associativeQualificationLocked || undefined}>
              ...
              >
                <div className={`investor-proposal-guide-card${usesDirectDesign ? " investor-direct-proposal-guide-card" : ""}`}>
                  <header>
                    <div><span>Orientação completa</span><h2 id="investor-proposal-guide-title">Vamos montar esta proposta juntos</h2></div>
                    <form method="dialog"><button type="submit" aria-label="Fechar passo a passo">×</button></form>
                  </header>
                  <p id="investor-proposal-guide-description">Pense na proposta como uma conta montada em blocos. Você confere um bloco por vez, e o simulador usa cada resposta para calcular o próximo. Siga a ordem abaixo sem pular etapas.</p>
                  <ol>
                    {directTable ? DIRECT_TABLE_PROPOSAL_GUIDE_STEPS.map((step, index) => <li key={step.title}>
                      <span>{index + 1}</span>
                      <div><strong>{step.title}</strong><p>{step.description}</p><small>{step.note}</small></div>
                    </li>) : annualMode ? ASSOCIATIVE_PROPOSAL_GUIDE_STEPS.map((step, index) => <li key={step.title}>
                      <span>{index + 1}</span>
                      <div><strong>{step.title}</strong><p>{step.description}</p><small>{step.note}</small></div>
                    </li>) : <>
                      <li><span>1</span><div><strong>Confirme a unidade escolhida</strong><p>O valor do imóvel e a data de entrega vêm do estoque e ficam bloqueados. Se a unidade estiver incorreta, volte ao item 01 e selecione outra linha.</p></div></li>
                      <li><span>2</span><div><strong>Ajuste a entrada</strong><p>O campo inicia com 10% e pode ser editado em reais. O pagamento inicial nunca pode ficar abaixo de 6%; o percentual aparece abaixo do campo.</p></div></li>
                      <li><span>3</span><div><strong>Complete com sinais quando necessário</strong><p>Se a entrada ficar entre 6% e menos de 10%, o simulador distribui o valor restante em até 3 sinais, com datas comerciais nos dias 05, 10 ou 15. Cada sinal deve ser de no mínimo R$ 150,00; o Sinal 2 não pode superar o Sinal 1 e o Sinal 3 não pode superar o Sinal 2. Você pode ajustar os valores.</p></div></li>
                      <li><span>4</span><div><strong>Inclua intermediárias se o cliente desejar</strong><p>Cada intermediária pode representar até 5% do valor do imóvel. O limite é de 3 pagamentos no fluxo de 18 parcelas e 4 no fluxo de 24, sempre respeitando as datas e o prazo da obra.</p></div></li>
                      <li><span>5</span><div><strong>Aplique desconto somente com autorização</strong><p>Abra o campo apenas quando houver solicitação e autorização. O desconto reduz o valor real usado no cálculo e pode alterar percentuais e limites.</p></div></li>
                      <li><span>6</span><div><strong>Defina a quantidade de parcelas</strong><p>Entrada total de 10% a 19,99% permite até 18 parcelas; a partir de 20%, até 24. O prazo da obra pode reduzir esse máximo. Confira o valor mensal e a data da primeira parcela no cartão.</p></div></li>
                      <li><span>7</span><div><strong>Valide o resultado</strong><p>No item 04, confira composição, saldo, parcela mensal e datas. Abra a auditoria e corrija cada reprovação indicada antes de imprimir ou apresentar ao cliente.</p></div></li>
                    </>}
                  </ol>
                  <aside><strong>Regra simples para não se perder</strong><p>{directTable ? "Leia sempre de cima para baixo: imóvel → renda → opção → pagamentos → parcela pós-chaves → resultado → auditoria. Se uma linha reprovar, corrija essa linha antes de continuar." : annualMode ? "Ela evita montar um fluxo fora da regra e permite visualizar as datas de sinais, anuais e parcelas antes de apresentar a proposta." : "Ela evita montar um fluxo fora da regra e permite visualizar as datas de sinais, intermediárias e parcelas antes de apresentar a proposta."}</p></aside>
                  <form method="dialog" className="investor-proposal-guide-actions"><button type="submit">Entendi, montar proposta</button></form>
                </div>
              </dialog>

              {directTable ? <div className={`investor-direct-credit-tour-anchor investor-direct-editable-freeze${directIncomeReady ? "" : " is-locked"}`}>
                {!directIncomeReady ? <div className="investor-direct-editable-lock" role="status" aria-live="polite">
                  ...
                  <div>
                    ...
                    <small>A opção 01 permanece como referência. Nenhum campo abaixo pode ser alterado até o preenchimento da renda.</small>
                  </div>
                ...
                <fieldset disabled={!directIncomeReady} aria-disabled={!directIncomeReady}>
                  <legend className="sr-only">Fluxo editável da proposta {directIncomeReady ? "liberado" : "congelado até informar a renda"}</legend>
                  <div className="investor-direct-account investor-direct-editable-account" role="group" aria-labelledby="investor-flow-title">
                <div className="investor-direct-account-legend" aria-hidden="true"><span>Etapa</span><span>Cálculo</span><span>Resultado</span></div>
                  <ol>
                  ...
                  <DirectEditableAccountRow
                    ...
                    result={<strong>{money.format(result.context.valueReal - result.custom.actValue)}</strong>}
                  />
                  {visibleSignalIndexes.map((index, visiblePosition) => {
                    ...
                    const hideActionText = index === signals.length - 1 ? `Ocultar ${index + 1}` : `Ocultar ${index + 1}–${signals.length}`;
                    return <DirectEditableAccountRow
                      key={`editable-signal-${index}`}
                      number={5 + visiblePosition}
                      operator={active && !signal.approved ? "!" : "−"}
                      label={`Sinal ${index + 1}`}
                      meta={active ? `Pagamento em ${formatDate(signal.date)}` : `Previsto para ${formatDate(signal.date)}`}
                      calculation={active ? `${money.format(result.context.valueReal)} × ${percent.format(signal.rate)}` : "Complete se necessário"}
                      invalid={active && !signal.approved}
                      result={<div className="investor-direct-editable-value has-row-action"><span aria-hidden="true">R$</span><MoneyInput inputRef={(input) => { signalInputRefs.current[index] = input; }} label={`Valor do sinal ${index + 1}`} describedBy={statusId} invalid={active && !signal.approved} value={value} onChange={(nextValue) => updateSignal(index, nextValue)} /><button type="button" className="investor-direct-row-hide" aria-label={hideActionLabel} title={hideActionLabel} onClick={() => hideSignalField(index)}><span aria-hidden="true">×</span><span>{hideActionText}</span></button><small id={statusId} role={active && !signal.approved ? "alert" : undefined}>{active ? signal.reason : "Campo opcional"}</small></div>}
                    />;
                  })}
                  <DirectEditableAccountRow
                  ...
                  {visibleIntermediaryIndexes.map((index, visiblePosition) => {
                    ...
                    return <DirectEditableAccountRow
                      ...
                      label={`Intermediária ${item.index}`}
                      meta={item.date ? `Pagamento em ${formatDate(item.date)}` : "Data indisponível"}
                      calculation={item.value > 0 ? `${money.format(result.context.valueReal)} × ${percent.format(item.rate)}` : `Limite: ${money.format(result.context.valueReal)} × 5,0%`}
                      invalid={item.value > 0 && !item.approved}
                      result={<div className="investor-direct-editable-value has-row-action"><span aria-hidden="true">R$</span><MoneyInput inputRef={(input) => { intermediaryInputRefs.current[index] = input; }} label={`Valor da intermediária ${item.index}`} describedBy={statusId} invalid={item.value > 0 && !item.approved} max={result.context.valueReal * 0.05} value={intermediaries[index]} onChange={(nextValue) => updateIntermediary(index, nextValue)} /><button type="button" className="investor-direct-row-hide" aria-label={`Ocultar Intermediária ${item.index} e zerar valor`} title={`Ocultar Intermediária ${item.index}`} onClick={() => hideIntermediaryField(index)}>Ocultar</button><small id={statusId} role={item.value > 0 && !item.approved ? "alert" : undefined}>{item.value > 0 ? item.reason : `Máximo ${money.format(result.context.valueReal * 0.05)}`}</small></div>}
                    />
                  })}
                  <DirectEditableAccountRow
                    label={intermediariesVisible ? "Adicionar intermediária" : "Intermediárias"}
                    tourTarget="proposal-intermediaries"
                    meta={intermediaryFieldLimit > 0 ? `5% cada · ${intermediaryFieldLimit} disponíveis até ${formatDate(result.context.deadline)} (3 meses antes da entrega)` : `Nenhuma data disponível até ${formatDate(result.context.deadline)} (3 meses antes da entrega)`}
                    calculation={`Total válido: ${money.format(result.custom.validIntermediaryTotal)}`}
                    result={<div className="investor-direct-editable-actions">
                      {visibleIntermediaryCount < intermediaryFieldLimit ? <button ref={intermediaryActionRef} type="button" aria-label={intermediariesVisible ? "Adicionar ou reexibir intermediária" : "Adicionar intermediária"} title={intermediariesVisible ? "Adicionar ou reexibir intermediária" : "Adicionar intermediária"} onClick={addIntermediaryField}><span aria-hidden="true">+</span></button> : <small>{intermediaryFieldLimit > 0 ? "Limite da entrega atingido" : "Sem datas antes do limite"}</small>}
                    </div>}
                  />
                  <DirectEditableAccountRow
                  ...
                  <DirectEditableAccountRow
                    ...
                    action
                    total
                    result={<><strong>{money.format(result.custom.postKeysPayment)}</strong><button type="button" className="investor-info-trigger investor-direct-dialog-trigger" aria-label={`Ver amortização das ${result.custom.postKeysInstallments} parcelas pós-chaves`} title={`Ver amortização das ${result.custom.postKeysInstallments} parcelas pós-chaves`} aria-haspopup="dialog" aria-controls="investor-direct-amortization" onClick={() => directAmortizationDialog.current?.showModal()}><span className="investor-info-mark" aria-hidden="true" /></button></>}
                  />
                  <DirectEditableAccountRow
              ...
              {directVisualLayout ? <div className="investor-associative-workspace">
                <div className="investor-associative-ledger" role="group" aria-labelledby="investor-flow-title">
                <div className="investor-direct-account investor-direct-editable-account investor-associative-compact-account">
                  <ol onKeyDown={focusNextAssociativeRow}>
                    ...
                    <AssociativeEditableAccountRow
                      ...
                      meta={associativeHelp("Digite somente o financiamento aprovado pelo banco. Um valor maior que zero libera o Subsídio.", `Saldo atual: ${money.format(result.context.valueReal)} − recursos informados = ${money.format(result.context.balanceAfterResources)}.`)}
                      calculation={<AssociativeMoneyControl inputRef={associativeFinancingInputRef} label="Financiamento" value={financing} onChange={setFinancing} />}
                      sideGuidance={associativeGuidanceStage === "financing" ? <AssociativeStepGuide title="Informe o Financiamento" description="Digite um valor maior que zero para liberar o Subsídio." /> : null}
                    />
                    <AssociativeEditableAccountRow
                      number={5}
                      operator="−"
                      ...
                      meta={associativeHelp("Digite o subsídio aprovado. Se não houver, digite zero para liberar o FGTS.", `Saldo atual: ${money.format(result.context.valueReal)} − recursos informados = ${money.format(result.context.balanceAfterResources)}.`)}
                      calculation={<AssociativeMoneyControl label="Subsídio" value={subsidy} disabled={!associativeSubsidyUnlocked} onChange={setSubsidy} />}
                      sideGuidance={associativeGuidanceStage === "subsidy" ? <AssociativeStepGuide title="Informe o Subsídio" description="Digite o valor aprovado ou zero quando não houver para liberar o FGTS." /> : null}
                    />
                    <AssociativeEditableAccountRow
                      number={6}
                      operator="−"
                      ...
                      meta={associativeHelp("Digite o FGTS que será usado. Se não houver, digite zero para liberar o Cheque Moradia.", `Saldo atual: ${money.format(result.context.valueReal)} − recursos informados = ${money.format(result.context.balanceAfterResources)}.`)}
                      calculation={<AssociativeMoneyControl label="FGTS" value={fgts} disabled={!associativeFgtsUnlocked} onChange={setFgts} />}
                      sideGuidance={associativeGuidanceStage === "fgts" ? <AssociativeStepGuide title="Informe o FGTS" description="Digite o valor utilizado ou zero quando não houver para liberar o Cheque Moradia." /> : null}
                    />
                    <AssociativeEditableAccountRow
                      number={7}
                      operator="−"
                      ...
                      meta={associativeHelp("Digite o valor do Cheque Moradia. Se não houver, digite zero para liberar a Entrada.", `Saldo atual: ${money.format(result.context.valueReal)} − recursos informados = ${money.format(result.context.balanceAfterResources)}.`)}
                      calculation={<AssociativeMoneyControl label="Cheque Moradia" value={housingCheck} disabled={!associativeHousingCheckUnlocked} onChange={setHousingCheck} />}
                      sideGuidance={associativeGuidanceStage === "housingCheck" ? <AssociativeStepGuide title="Informe o Cheque Moradia" description="Digite o valor utilizado ou zero quando não houver para liberar a Entrada." /> : null}
                    />

                    <AssociativeEditableAccountRow number={8} operator="=" label="Saldo após recursos" fieldState="locked" meta={associativeHelp("É o valor que sobra depois de tirar Financiamento, Subsídio, FGTS e Cheque Moradia.", `Conta atual: ${money.format(result.context.valueReal)} − ${money.format(result.custom.financing)} − ${money.format(result.custom.subsidy)} − ${money.format(result.custom.fgts)} − ${money.format(result.custom.housingCheck)} = ${money.format(result.context.balanceAfterResources)}.`)} calculation={<AssociativeMoneyValue label="Saldo após recursos" value={result.context.balanceAfterResources} />} total />
                    ...
                    <AssociativeEditableAccountRow
                      ...
                      sideGuidance={associativeGuidanceStage === "entry" ? <AssociativeStepGuide
                        title={associativeEntryRejected ? "Revise a Entrada" : "Informe a Entrada"}
                        tone={associativeEntryRejected ? "rejected" : "guidance"}
                        description={associativeEntryRejected
                    ...
                    <AssociativeEditableAccountRow
                      ...
                      sideGuidance={associativeGuidanceStage === "installments" ? <AssociativeStepGuide
                        ...
                        tone={associativeInstallmentsRejected && installments !== "" ? "rejected" : "guidance"}
                        description={associativeBlockDistributionError || `Use um número inteiro entre 1 e ${result.context.maxInstallments} para liberar o Ranking.`}
                      /> : null}
                    />
                  </ol>
                </div>
                ...
                {associativeRankingUnlocked ? <div className={`investor-associative-results-stack${associativeApprovalDetailsUnlocked ? " is-complete" : " is-ranking-only"}`}>
                  <AssociativeApprovalPanel
                    ...
                    decreasingMaximumIncomePayment={associativePaymentComparison.highestDecreasingTotal}
                    linearInstallmentDate={associativePaymentComparison.highestLinearPaymentRow?.paymentDate}
                    decreasingInstallmentDate={associativePaymentComparison.highestDecreasingPaymentRow?.paymentDate}
                    ...
                    currentAnnualPayments={result.custom.intermediaries
                      .map((item: { index: number; date: string; value: number }) => ({ label: `Anual ${item.index}`, date: item.date, value: item.value }))}
                  ...
                  />
                  {associativeApprovalDetailsUnlocked ? <AssociativePaymentSummary
                ...
                />
              </div> : <div className="investor-standard-ledger-workspace">
                <div className="investor-associative-ledger investor-standard-ledger" role="group" aria-labelledby="investor-flow-title">
                  <div className="investor-direct-account investor-direct-editable-account investor-associative-compact-account">
                    <ol onKeyDown={focusNextAssociativeRow}>
                      <li className="investor-associative-payment-actions-row" role="presentation">
                        <span className="investor-direct-step-number" aria-hidden="true" />
                        <div className="investor-associative-payment-actions-bar" aria-label="Adicionar pagamentos opcionais">
                          <button ref={signalActionRef} type="button" data-tour="proposal-signals" aria-describedby="investor-standard-signal-action-status" disabled={visibleSignalCount >= signals.length} title={visibleSignalCount >= signals.length ? "Limite de 3 sinais atingido" : "Adicionar sinal dentro das regras do fluxo"} onClick={addSignalField}>Inserir Sinal</button>
                          <button ref={intermediaryActionRef} type="button" data-tour="proposal-intermediaries" aria-describedby="investor-standard-intermediary-action-status" disabled={result.context.maxIntermediaries <= 0 || visibleIntermediaryCount >= result.context.maxIntermediaries} title={result.context.maxIntermediaries <= 0 ? "Complete 10% de entrada para liberar intermediárias" : visibleIntermediaryCount >= result.context.maxIntermediaries ? `Limite de ${result.context.maxIntermediaries} intermediárias atingido` : "Adicionar intermediária dentro do limite válido"} onClick={addIntermediaryField}>Inserir Intermediária</button>
                          <button type="button" data-tour="proposal-discount" aria-pressed={discountAuthorized} onClick={toggleDiscountField}>{discountAuthorized ? "Remover Desconto" : "Inserir Desconto"}</button>
                          <span className="sr-only" id="investor-standard-signal-action-status" aria-live="polite">{visibleSignalCount >= signals.length ? "Limite de 3 sinais atingido." : `${signals.length - visibleSignalCount} sinais ainda podem ser incluídos.`}</span>
                          <span className="sr-only" id="investor-standard-intermediary-action-status" aria-live="polite">{result.context.maxIntermediaries <= 0 ? "Complete 10% de entrada total para liberar intermediárias." : visibleIntermediaryCount >= result.context.maxIntermediaries ? `Limite de ${result.context.maxIntermediaries} intermediárias atingido.` : `${result.context.maxIntermediaries - visibleIntermediaryCount} intermediárias ainda podem ser incluídas.`}</span>
                        </div>
                      </li>

                      <AssociativeEditableAccountRow number={1} operator="=" label="Valor do imóvel" fieldState="locked" meta="Valor original da unidade escolhida no estoque." calculation={<AssociativeMoneyValue label="Valor do imóvel" value={result.context.salePrice} />} />
                      {discountAuthorized ? <AssociativeEditableAccountRow
                        operator="−"
                        label="Desconto"
                        fieldState="editable"
                        rowClassName="investor-associative-discount-line"
                        meta="Desconto comercial opcional. Use somente quando estiver autorizado; ele reduz o valor real da venda."
                        calculation={<AssociativeMoneyControl inputRef={discountInputRef} label="Desconto autorizado" max={Math.max(0, result.context.salePrice - 0.01)} value={discount} onChange={setDiscount} />}
                      /> : null}
                      <AssociativeEditableAccountRow number={2} operator="=" label="Valor real da venda" fieldState="locked" meta="Valor do imóvel menos o desconto autorizado." calculation={<AssociativeMoneyValue label="Valor real da venda" value={result.context.valueReal} />} total />
                      <AssociativeEditableAccountRow
                        number={3}
                        operator="−"
                        label="Entrada"
                        ...
                        meta={`Mínimo de ${money.format(result.context.valueReal * 0.06)} (6%). Com sinais, a entrada total precisa atingir 10%; a partir de 20%, o plano pode chegar a 24 parcelas.`}
                        calculation={<><AssociativeMoneyControl label="Entrada" describedBy="investor-entry-meta" invalid={result.custom.actRate < 0.06} max={Math.max(0, result.context.valueReal - 0.01)} value={entryValue} onChange={updateEntryValue} /><span className="sr-only" id="investor-entry-meta" role={result.custom.actRate < 0.06 ? "alert" : "status"} aria-live="polite">{result.custom.actRate < 0.06 ? `Entrada abaixo do mínimo de ${money.format(result.context.valueReal * 0.06)}` : `Entrada corresponde a ${percent.format(result.custom.actRate)} do valor real`}</span></>}
                      />
                      ...
                      {visibleSignalIndexes.map((index) => {
                        ...
                        const signal = result.custom.signals[index];
                        const active = currencyInputNumber(value) > 0;
                        const invalid = active && !signal.approved;
                        const otherSignals = signals.reduce((total, current, currentIndex) => currentIndex === index ? total : total + currencyInputNumber(current), 0);
                        const previousSignalLimit = index === 0 ? result.context.valueReal : currencyInputNumber(signals[index - 1]);
                        const remainingLimit = Math.max(0, (Math.round((result.context.valueReal - result.custom.actValue - result.custom.validIntermediaryTotal - otherSignals) * 100) - 1) / 100);
                        const maximumSignalValue = Math.max(0, Math.min(previousSignalLimit, remainingLimit));
                        const statusId = `investor-standard-signal-${index + 1}-status`;
                        return <AssociativeEditableAccountRow
                          key={index}
                          operator={invalid ? "!" : "−"}
                          label={`Sinal ${index + 1}`}
                          ...
                          rowClassName="payment-group-child payment-group-child-signal"
                          fieldState="editable"
                          leadingAction={<button type="button" className="investor-associative-row-remove" aria-label={`Ocultar Sinal ${index + 1} e zerar os sinais seguintes`} title={`Ocultar Sinal ${index + 1}`} onClick={() => hideSignalField(index)}><span aria-hidden="true">×</span></button>}
                          meta={`Mínimo de ${money.format(150)}. Máximo válido nesta composição: ${money.format(maximumSignalValue)}. Os sinais são consecutivos e não podem crescer.`}
                          calculation={<><AssociativeMoneyControl inputRef={(input) => { signalInputRefs.current[index] = input; }} label={`Sinal ${index + 1}`} describedBy={statusId} invalid={invalid} max={maximumSignalValue} limitToMax value={value} onChange={(nextValue) => updateSignal(index, nextValue)} /><span className="sr-only" id={statusId} role={invalid ? "alert" : "status"} aria-live="polite">{active ? signal.reason : "Sinal opcional não usado"}</span></>}
                          invalid={invalid}
                        />;
                      })}
                      ...
                      {visibleIntermediaryIndexes.map((index) => {
                        const item = result.custom.intermediaries[index];
                        const invalid = item.value > 0 && !item.approved;
                        const remainingLimit = Math.max(0, (Math.round((result.custom.balance + (item.approved ? item.value : 0)) * 100) - 1) / 100);
                        const percentageLimit = Math.floor(result.context.valueReal * 0.05 * 100) / 100;
                        const maximumIntermediaryValue = Math.max(0, Math.min(percentageLimit, remainingLimit));
                        const statusId = `investor-standard-intermediary-${item.index}-status`;
                        return <AssociativeEditableAccountRow
                          key={item.index}
                          operator={invalid ? "!" : "−"}
                          label={`Intermediária ${item.index}`}
                          ...
                          rowClassName="payment-group-child payment-group-child-annual"
                          fieldState="editable"
                          leadingAction={<button type="button" className="investor-associative-row-remove" aria-label={`Ocultar Intermediária ${item.index} e zerar valor`} title={`Ocultar Intermediária ${item.index}`} onClick={() => hideIntermediaryField(index)}><span aria-hidden="true">×</span></button>}
                          meta={`Máximo válido de ${money.format(maximumIntermediaryValue)} (até 5% do valor real), respeitando a data limite de ${formatDate(result.context.deadline)}.`}
                          calculation={<><AssociativeMoneyControl inputRef={(input) => { intermediaryInputRefs.current[index] = input; }} label={`Intermediária ${item.index}`} describedBy={statusId} invalid={invalid} max={maximumIntermediaryValue} limitToMax value={intermediaries[index]} onChange={(nextValue) => updateIntermediary(index, nextValue)} /><span className="sr-only" id={statusId} role={invalid ? "alert" : "status"} aria-live="polite">{item.value > 0 ? item.reason : "Intermediária opcional não usada"}</span></>}
                          invalid={invalid}
                        />;
                      })}

                      <AssociativeEditableAccountRow number={8} operator="=" label="Saldo parcelado" fieldState="locked" meta={`Valor real menos entrada, sinais válidos e intermediárias válidas. Total dos sinais: ${money.format(result.custom.signalTotal)}; total das intermediárias: ${money.format(result.custom.validIntermediaryTotal)}.`} calculation={<AssociativeMoneyValue label="Saldo parcelado" value={result.custom.balance} />} total />
                      <AssociativeEditableAccountRow
                        number={9}
                        operator="÷"
                        label="Qtd. de parcelas"
                        ...
                        meta={`Use um número inteiro entre 1 e ${result.context.maxInstallments || 0}. O máximo depende da entrada total e do prazo da obra.`}
                        calculation={<div className="investor-direct-editable-value investor-associative-line-control investor-standard-installment-control"><span aria-hidden="true">×</span><input aria-label="Quantidade de parcelas" name="quantidade-de-parcelas-investidor" autoComplete="off" aria-describedby="investor-installment-guidance" aria-invalid={!validInstallmentSchedule || undefined} type="number" min="1" max={result.context.maxInstallments || 1} step="1" value={result.context.maxInstallments > 0 ? installments : ""} placeholder="0" disabled={result.context.maxInstallments <= 0} onChange={(event) => setInstallments(event.target.value)} /></div>}
                      />
                      <AssociativeEditableAccountRow number={10} operator="=" label="Parcela mensal" fieldState="locked" date={firstInstallmentDate || undefined} dateLabel="1ª em" meta={`Última mensal em ${formatPaymentDate(lastInstallmentDate)}. O fechamento preserva os centavos na última parcela.`} calculation={<div className="investor-standard-monthly-value"><strong>{installmentSummaryFromBalance(result.custom.desiredInstallments, result.custom.balance)}</strong></div>} total />
                    </ol>
                  </div>
                </div>
              </div>}
          ...
          {annualMode ? <>
            <section className="investor-direct-resource-actions investor-associative-resource-actions" aria-labelledby="investor-associative-resources-title" data-tour="documents">
              ...
              <button type="button" aria-haspopup="dialog" aria-controls="investor-documentation-pf" onClick={() => pfDocumentationDialog.current?.showModal()}>Doc Pessoa Física</button>
              <button type="button" onClick={() => window.print()} aria-label="Imprimir a proposta associativa">Imprimir</button>
              <InvestorCommercialLinks />
            </section>
            <DocumentationDialog type="pf" dialogRef={pfDocumentationDialog} associative />
          </> : directTable ? <>
            <section className="investor-direct-resource-actions" aria-labelledby="investor-direct-resources-title" data-tour="resources">
              <h2 id="investor-direct-resources-title" className="sr-only">Ajuda e documentação da Tabela Direta</h2>
