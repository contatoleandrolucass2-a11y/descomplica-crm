...
export function calculateAssociativeLinearArchive(
  ...
  const signal2Date =
    signal2 >= 150 && signal2Valid && signal1Date ? latestOfficialDate(signal1Date, 31) : null;
  const signal3Date =
    signal3 >= 150 && signal3Valid && signal2Date ? latestOfficialDate(signal2Date, 31) : null;
  ...
  const firstInstallmentDate =
    entry >= 150 && installmentBaseDate ? addMonths(installmentBaseDate, graceMonths) : null;
  if (entryDate && !baseSignalDate)
    errors.push("Não foi possível localizar uma data válida nos dias 5, 10 ou 15.");
  if (entry >= 150 && !firstInstallmentDate)
    errors.push("Não foi possível definir a data de início das mensais.");

  const validInitialTotal =
    entry +
    (signal1Valid && signal1 >= 150 ? signal1 : 0) +
    (signal2Valid && signal2 >= 150 ? signal2 : 0) +
    (signal3Valid && signal3 >= 150 ? signal3 : 0);
  ...
  const proSoluto = Math.max(
    0,
    realSaleValue - financing - subsidy - fgts - housingCheck - validInitialTotal,
  );
  ...
  const preInstallments =
    firstInstallmentDate && constructionEnd && installments > 0
      ? Math.max(0, Math.min(installments, monthsByCalendar(firstInstallmentDate, constructionEnd)))
      : 0;
  const postInstallments = Math.max(0, installments - preInstallments);
  const baseRate =
    preInstallments >= 1 ? PARAMETERS.preRate : postInstallments >= 1 ? PARAMETERS.postRate : 0;
  const correctedInstallmentBalance =
    installmentBalance * (1 + baseRate) * (1 + baseRate) ** graceMonths;
  const preVariable =
    preInstallments === 0 ? 0 : (1.005 ** preInstallments * 0.005) / (1.005 ** preInstallments - 1);
  const postVariable =
    postInstallments === 0
      ? 0
      : 1.005 ** preInstallments *
        ((1.015 ** postInstallments * 0.015) / (1.015 ** postInstallments - 1));
  const prePercentage =
    preInstallments === 0
      ? 0
      : postInstallments === 0
  ...
  if (Math.abs(prePercentage + postPercentage - Number(installments > 0)) > 0.000001)
  ...
  const audit = [
    ...
    { label: "D59 + D60 fecha D61", ok: preInstallments + postInstallments === installments },
    {
      ...
      ok: Math.abs(prePercentage + postPercentage - Number(installments > 0)) <= 0.000001,
    },
    {
    ...
    },
    {
      ...
      ok: Boolean(
        ...
        PARAMETERS.paymentDays.includes(firstInstallmentDate.getUTCDate() as 5 | 10 | 15),
      ),
    },
  ];
  return {
