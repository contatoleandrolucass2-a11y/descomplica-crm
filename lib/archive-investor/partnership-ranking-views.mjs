export function latestPartnershipPeriod(entries) {
  return Array.from(new Set(entries.map((entry) => entry.periodMonth))).sort().at(-1) ?? "";
}
...
export function partnershipImobRows(entries, query) {
  ...
  return normalizedQuery
    ? positioned.filter(({ entry }) => normalizePartnershipSearch(entry.imobName).includes(normalizedQuery))
    : positioned.slice(0, 10);
}

export function partnershipPositionChanges(currentEntries, previousEntries, keyForEntry) {
  const previousPositions = new Map((previousEntries ?? []).map((entry, index) => [
    String(keyForEntry(entry)),
    index + 1,
  ]));

  return new Map((currentEntries ?? []).map((entry, index) => {
...
export function partnershipEntriesForView(entries, view, options = {}) {
  ...
  if (view === "custom") {
    ...
    const totals = new Map();
    for (const entry of entries.filter((item) => item.periodMonth >= startMonth && item.periodMonth <= endMonth)) {
      const current = totals.get(entry.imobKey) ?? {
  ...
  const selectedMonth = view === "previous"
    ? previousPartnershipPeriod(latestMonth)
    : latestMonth;
...
export function partnershipTotalsMatch(imobEntries, developmentEntries) {
  ...
  return imobs.contracts === developments.contracts
    && Math.abs(imobs.vgv - developments.vgv) < 0.01;
}

export function partnershipSnapshotsAligned(primaryAt, secondaryAt, maxSkewMs = DEFAULT_MAX_SOURCE_SKEW_MS) {
...
export function aggregatePartnershipDevelopmentSales(records) {
  ...
  for (const record of records ?? []) {
    const salesChannel = String(record?.salesChannel ?? "").trim().toLocaleUpperCase("pt-BR");
    ...
    const current = totals.get(key) ?? { periodMonth, businessUnit, developmentName, vgv: 0, contracts: 0 };
    current.vgv += amount;
    current.contracts += 1;
...
export function partnershipDevelopmentsForView(entries, view, options = {}) {
  ...
  if (view === "custom") {
    ...
    if (!startMonth || !endMonth) return [];
    const totals = new Map();
    for (const entry of scoped.filter((item) => item.periodMonth >= startMonth && item.periodMonth <= endMonth)) {
      const key = entry.developmentName.toLocaleLowerCase("pt-BR");
      const current = totals.get(key) ?? {
        periodMonth: startMonth,
        businessUnit: entry.businessUnit,
