...
export function calculateAssociativeLinear(raw, options = {}) {
  ...
  if (entry < ASSOCIATIVE_LINEAR_PARAMETERS.minimumEntryOrSignal)
    errors.push("A entrada deve ser de pelo menos R$ 150,00.");
  if (!raw.policyConfirmed)
  ...
  const signal1Valid =
    signal1 === 0 || signal1 >= ASSOCIATIVE_LINEAR_PARAMETERS.minimumEntryOrSignal;
  const signal2Valid =
    signal2 === 0 ||
    (signal1 >= ASSOCIATIVE_LINEAR_PARAMETERS.minimumEntryOrSignal &&
      signal2 >= ASSOCIATIVE_LINEAR_PARAMETERS.minimumEntryOrSignal &&
      signal2 <= signal1);
  const signal3Valid =
    signal3 === 0 ||
    (signal2 >= ASSOCIATIVE_LINEAR_PARAMETERS.minimumEntryOrSignal &&
      signal3 >= ASSOCIATIVE_LINEAR_PARAMETERS.minimumEntryOrSignal &&
      signal3 <= signal2);
  ...
  const signal2Date =
    signal2 >= 150 && signal2Valid && signal1Date ? latestOfficialDate(signal1Date) : null;
  const signal3Date =
    signal3 >= 150 && signal3Valid && signal2Date ? latestOfficialDate(signal2Date) : null;
  ...
  const firstInstallmentDate =
    entry >= ASSOCIATIVE_LINEAR_PARAMETERS.minimumEntryOrSignal && installmentBaseDate
  ...
  if (entry >= ASSOCIATIVE_LINEAR_PARAMETERS.minimumEntryOrSignal && !firstInstallmentDate)
  ...
  const annualInputs = [raw.annual1, raw.annual2, raw.annual3, raw.annual4, raw.annual5].map(
    number,
  );
  const annualSchedule = annualInputs.map((amount, index) => {
    ...
    if (amount > 0) {
      if (!dueDate || !constructionEnd || dueDate > constructionEnd)
    ...
    const months =
      valid && amount > 0 && calculationDate && dueDate
        ? fullMonthsBetween(calculationDate, dueDate)
        : 0;
    const corrected =
      valid && amount > 0
        ? amount *
          (1 + ASSOCIATIVE_LINEAR_PARAMETERS.annualRate) *
          Math.pow(1 + ASSOCIATIVE_LINEAR_PARAMETERS.annualRate, months)
        : 0;
  ...
  const annualCorrectedTotal = annualSchedule.reduce(
    (total, annual) => total + annual.corrected,
    0,
  );
  ...
  if (installments > 0 && proSoluto > 0 && annualCorrectedTotal >= proSoluto) {
    errors.push(
      "O total corrigido das anuais deve ser menor que o Pró-Soluto para preservar ao menos uma parcela mensal.",
    );
  }
  ...
  if (firstInstallmentDate && constructionEnd && installments > 0) {
    preInstallments = Math.max(
      0,
      Math.min(installments, monthsByCalendar(firstInstallmentDate, constructionEnd)),
    );
  }
  ...
  const correctedInstallmentBalance =
    installmentBalanceWithInitialCorrection * Math.pow(1 + baseRate, graceMonths);

  const preVariable =
    preInstallments === 0
      ? 0
      : (Math.pow(1 + ASSOCIATIVE_LINEAR_PARAMETERS.preRate, preInstallments) *
          ASSOCIATIVE_LINEAR_PARAMETERS.preRate) /
        (Math.pow(1 + ASSOCIATIVE_LINEAR_PARAMETERS.preRate, preInstallments) - 1);
  const postVariable =
    postInstallments === 0
      ? 0
      : Math.pow(1 + ASSOCIATIVE_LINEAR_PARAMETERS.preRate, preInstallments) *
        ((Math.pow(1 + ASSOCIATIVE_LINEAR_PARAMETERS.postRate, postInstallments) *
          ASSOCIATIVE_LINEAR_PARAMETERS.postRate) /
          (Math.pow(1 + ASSOCIATIVE_LINEAR_PARAMETERS.postRate, postInstallments) - 1));
  const prePercentage =
    preInstallments === 0
      ? 0
      : postInstallments === 0
  ...
  const adjustedPost =
    postPeriodTotal * Math.pow(1 + ASSOCIATIVE_LINEAR_PARAMETERS.preRate, preInstallments);
  const prePayment =
    preInstallments > 0
      ? -pmt(ASSOCIATIVE_LINEAR_PARAMETERS.preRate, preInstallments, adjustedPre)
      : 0;
  const postPayment =
    postInstallments > 0
      ? -pmt(ASSOCIATIVE_LINEAR_PARAMETERS.postRate, postInstallments, adjustedPost)
      : 0;
  ...
  if (Math.abs(prePercentage + postPercentage - (installments > 0 ? 1 : 0)) > 0.000001)
  ...
  const audit = [
    { label: "Executor WF-13 e escopo Associativo | Fluxo Linear", ok: true },
    {
      ...
      ok: Boolean(raw.development?.trim() && raw.product?.trim() && raw.stockMatch),
    },
    {
      label: "Política comercial conferida",
      ok: Boolean(raw.policyConfirmed && policyLimit > 0 && installments <= policyLimit),
    },
    {
      label: "Entrada e sinais respeitam a sequência mínima",
      ok: entry >= 150 && signal1Valid && signal2Valid && signal3Valid,
    },
    { label: "D59 + D60 fecha D61", ok: preInstallments + postInstallments === installments },
    {
      label: "F59 + F60 fecha 100%",
      ok: Math.abs(prePercentage + postPercentage - (installments > 0 ? 1 : 0)) <= 0.000001,
    },
    {
      label: "Períodos pré e pós fecham a base mensal corrigida",
      ok: Math.abs(prePeriodTotal + postPeriodTotal - correctedInstallmentBalance) <= 0.01,
    },
    {
      label: "Parcela final acompanha data 5/10/15",
      ok: Boolean(
        firstInstallmentDate &&
        ASSOCIATIVE_LINEAR_PARAMETERS.paymentDays.includes(firstInstallmentDate.getUTCDate()),
      ),
    },
  ];
