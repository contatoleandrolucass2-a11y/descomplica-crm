...
export function calculateAssociativeDecreasing(raw) {
  ...
  if (![rawInstallments, rawPreInstallments, rawPostInstallments].every(Number.isInteger)) errors.push("As quantidades de parcelas devem ser inteiras.");
  if (preInstallments + postInstallments !== installments) errors.push("A soma das parcelas pré e pós não fecha a quantidade total.");
  ...
  const blockCounts = [regularBlockCount, regularBlockCount, regularBlockCount, Math.max(0, installments - regularBlockCount * 3)];
  ...
  const blocks = ASSOCIATIVE_DECREASING_PARAMETERS.percentages.map((percentage, index) => {
    ...
    const preVariable = pre === 0
      ? 0
      : (prePower * ASSOCIATIVE_DECREASING_PARAMETERS.preRate) / (prePower - 1);
    const postVariable = post === 0
      ? 0
      : prePower * ((postPower * ASSOCIATIVE_DECREASING_PARAMETERS.postRate) / (postPower - 1));
    const prePercentage = pre === 0
      ? 0
      : post === 0
        ? 1
    ...
    const preAdjustment = index === 1
      ...
      : 1;
    const postAdjustment = Math.pow(1 + ASSOCIATIVE_DECREASING_PARAMETERS.preRate, accumulatedPre + pre)
      * Math.pow(1 + ASSOCIATIVE_DECREASING_PARAMETERS.postRate, accumulatedPost);
    const adjustedPre = prePrincipal * preAdjustment;
    const adjustedPost = postPrincipal * postAdjustment;
    const prePayment = pre > 0 ? pmt(ASSOCIATIVE_DECREASING_PARAMETERS.preRate, pre, adjustedPre) : 0;
    const postPayment = post > 0 ? pmt(ASSOCIATIVE_DECREASING_PARAMETERS.postRate, post, adjustedPost) : 0;
    const correctedInstallment = Math.max(prePayment, postPayment, 0);
    const uncorrectedInstallment = count > 0 ? uncorrectedBase / count : 0;
    const startDate = firstInstallmentDate ? addMonthsClamped(firstInstallmentDate, elapsedInstallments) : null;
  ...
  const preCountClosed = blocks.reduce((total, block) => total + block.preInstallments, 0) === preInstallments;
  const postCountClosed = blocks.reduce((total, block) => total + block.postInstallments, 0) === postInstallments;
  const allBlocksPayable = blocks.every((block) => block.count > 0 && block.uncorrectedInstallment > 0 && block.correctedInstallment > 0);
  if (!blockCountClosed) errors.push("Os quatro blocos não fecham a quantidade total.");
  if (!preCountClosed || !postCountClosed) errors.push("A distribuição pré e pós dos blocos não fecha o plano.");
  if (!allBlocksPayable) errors.push("Os quatro blocos precisam ter quantidade e parcela positivas.");
  ...
  return {
    ...
    audit: [
      ...
      { label: "Os percentuais monetários fecham 100%", ok: Math.abs(blocks.reduce((total, block) => total + block.percentage, 0) - 1) < 0.000001 },
    ],
  };
}
