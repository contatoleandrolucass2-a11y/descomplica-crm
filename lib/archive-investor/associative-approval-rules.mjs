...
export function distributeAssociativeApprovalPayment(
  ...
  const lastRequiredSignal = signalFloorCents.reduce(
...
export function calculateAssociativeApproval({
  ...
  const safeDecreasingInstallment = nonNegativeNumber(
    decreasingInstallment ?? correctedInstallment,
  );
  const safeInstallment = Math.max(safeLinearInstallment, safeDecreasingInstallment);
  const safeLinearMaximumIncomePayment = nonNegativeNumber(
    linearMaximumIncomePayment ?? linearInstallment,
  );
  const safeDecreasingMaximumIncomePayment = nonNegativeNumber(
    decreasingMaximumIncomePayment ?? decreasingInstallment,
  );
  ...
  const decreasingMaximumIncomeRate =
    safeIncome > 0 ? safeDecreasingMaximumIncomePayment / safeIncome : 0;
    ...
    ? [
        {
          ...
          ok: proSolutoRate <= tier.proSolutoRate,
        },
        {
          id: "commitment",
          value: commitmentRate,
          limit: tier.commitmentRate,
          ok: commitmentRate <= tier.commitmentRate,
        },
        {
          id: "annual-income",
          value: annualIncomeRate,
          limit: tier.annualIncomeLimitRate,
          ok: annualIncomeRate <= tier.annualIncomeLimitRate,
        },
      ]
    : [];
