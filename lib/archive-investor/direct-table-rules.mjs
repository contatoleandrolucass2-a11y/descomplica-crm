...
function firstPostKeysPayment(
  ...
  const basePaymentCents = Math.round(pricePayment(fromCents(principalCents), periods) * 100);
  const mipCents = Math.round(principalCents * DIRECT_TABLE_PARAMETERS.mipRate);
  const dfiCents = Math.round(principalCents * DIRECT_TABLE_PARAMETERS.dfiRate);
  return fromCents(basePaymentCents + mipCents + dfiCents);
}
...
export function buildDirectTableAmortizationSchedule(
  ...
  const principalCents = toCents(principalInput);
  ...
  const schedule = [];
  let openingBalanceCents = principalCents;
  let cumulativeTotalPaymentsCents = 0;

  for (let month = 1; month <= periods; month += 1) {
    const interestCents = Math.round(openingBalanceCents * monthlyRate);
    const regularAmortizationCents = Math.max(0, basePaymentCents - interestCents);
    const amortizationCents =
      month === periods
        ? openingBalanceCents
        : Math.min(openingBalanceCents, regularAmortizationCents);
    const mipCents = Math.round(openingBalanceCents * DIRECT_TABLE_PARAMETERS.mipRate);
    const dfiBaseCents =
      month === 1 ? openingBalanceCents : openingBalanceCents + cumulativeTotalPaymentsCents;
    const dfiCents = Math.round(dfiBaseCents * DIRECT_TABLE_PARAMETERS.dfiRate);
    const totalPaymentCents = amortizationCents + interestCents + mipCents + dfiCents;
    const closingBalanceCents = Math.max(0, openingBalanceCents - amortizationCents);
    ...
    schedule.push({
      ...
      mip: fromCents(mipCents),
      dfi: fromCents(dfiCents),
      totalPayment: fromCents(totalPaymentCents),
      balance: fromCents(closingBalanceCents),
    });
    cumulativeTotalPaymentsCents += totalPaymentCents;
...
export function buildDirectTablePreKeysSchedule(
  ...
  const principalCents = Math.max(0, Math.round((Number(principalInput) || 0) * 100));
  const installmentCount = Math.max(0, Math.trunc(Number(installmentCountInput) || 0));
  const firstPaymentDate = parseIsoDate(firstPaymentDateInput);
  if (principalCents <= 0 || installmentCount <= 0) return [];

  const roundedAverageCents = Math.round(principalCents / installmentCount);
  const regularPaymentCents =
    installmentCount === 1
      ? principalCents
      : Math.min(roundedAverageCents, Math.floor(principalCents / (installmentCount - 1)));
  const lastPaymentCents = principalCents - regularPaymentCents * (installmentCount - 1);
  let remainingCents = principalCents;
  return Array.from({ length: installmentCount }, (_, index) => {
    const paymentCents = index === installmentCount - 1 ? lastPaymentCents : regularPaymentCents;
