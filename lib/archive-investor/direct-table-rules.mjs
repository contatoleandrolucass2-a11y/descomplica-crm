export const DIRECT_TABLE_PARAMETERS = Object.freeze({
  ...
  parkingPreKeysRate: 0.40,
  parkingPostKeysRate: 0.50,
  parkingPostKeysInstallments: 66,
  annualInterest: 0.12,
  mipRate: 0.00021,
  dfiRate: 0.00007,
  creditCommitmentLimit: 0.40,
...
export function isDirectTableParkingPlant(plantInput) {
  return String(plantInput ?? "").normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLocaleLowerCase("pt-BR").includes("vaga");
}
...
export function resolveDirectTablePolicy(plantInput) {
  ...
  return Object.freeze({
    parking,
    preKeysRate: parking ? DIRECT_TABLE_PARAMETERS.parkingPreKeysRate : DIRECT_TABLE_PARAMETERS.preKeysRate,
    postKeysRate: parking ? DIRECT_TABLE_PARAMETERS.parkingPostKeysRate : DIRECT_TABLE_PARAMETERS.postKeysRate,
    postKeysInstallments: parking ? DIRECT_TABLE_PARAMETERS.parkingPostKeysInstallments : DIRECT_TABLE_PARAMETERS.postKeysInstallments,
  });
}
...
function pricePayment(principal, installmentCount = DIRECT_TABLE_PARAMETERS.postKeysInstallments) {
  ...
  const periods = Math.max(1, Math.trunc(Number(installmentCount) || DIRECT_TABLE_PARAMETERS.postKeysInstallments));
...
function firstPostKeysPayment(principal, installmentCount = DIRECT_TABLE_PARAMETERS.postKeysInstallments) {
  return pricePayment(principal, installmentCount)
    + (principal * DIRECT_TABLE_PARAMETERS.mipRate)
    + (principal * DIRECT_TABLE_PARAMETERS.dfiRate);
}

export function buildDirectTableAmortizationSchedule(principalInput, firstPaymentDateInput, installmentCountInput = DIRECT_TABLE_PARAMETERS.postKeysInstallments) {
  const principal = Math.max(0, Number(principalInput) || 0);
  if (!(principal > 0)) return [];
  const periods = Math.max(1, Math.trunc(Number(installmentCountInput) || DIRECT_TABLE_PARAMETERS.postKeysInstallments));
  const monthlyRate = Math.pow(1 + DIRECT_TABLE_PARAMETERS.annualInterest, 1 / 12) - 1;
  ...
  for (let month = 1; month <= periods; month += 1) {
    ...
    const paymentDate = firstPaymentDate ? toIsoDate(addUtcMonths(firstPaymentDate, month - 1)) : "";
    schedule.push({ month, paymentDate, amortization, interest, mip, dfi, totalPayment, balance: closingBalance });
...
export function buildDirectTablePreKeysSchedule(principalInput, installmentCountInput, firstPaymentDateInput) {
  const principal = Math.max(0, Number(principalInput) || 0);
...
export function calculateDirectTable(input) {
  ...
  if (propertyValue > 0 && discount >= propertyValue) errors.push("O desconto precisa ser menor que o valor do imóvel.");
  ...
  const audit = [
    ...
    { label: `Distribuição 10% + ${policy.preKeysRate * 100}% + ${policy.postKeysRate * 100}%`, ok: true },
  ...
  const scenarioOne = buildScenario({
    ...
    workEndDate,
    policy,
    payments: [{ label: "Ato", rate: DIRECT_TABLE_PARAMETERS.actMinimumRate, value: valueReal * DIRECT_TABLE_PARAMETERS.actMinimumRate }],
  });
  const scenarioTwo = buildScenario({
    ...
    policy,
    payments: [
      { label: "Ato", rate: DIRECT_TABLE_PARAMETERS.scenarioTwoActRate, value: valueReal * DIRECT_TABLE_PARAMETERS.scenarioTwoActRate },
      ...DIRECT_TABLE_PARAMETERS.scenarioTwoSignalRates.map((rate, index) => ({ label: `Sinal ${index + 1}`, rate, value: valueReal * rate })),
    ],
  });
...
export function formatDirectTableMessage(result, scenario) {
  ...
  const payments = scenario.payments
    ...
    .map((payment) => `${payment.label}: ${money.format(payment.value)}  Data: ${formatDate(payment.date)}`)
    .join("\n");
  return [
