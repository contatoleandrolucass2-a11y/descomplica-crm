...
export function resolveInvestorRegion(item = {}) {
  const explicitRegionValue = [item.region, item.regiao].find(
    (value) => normalizeLocationName(value) !== "",
  );
  const explicitRegion = EXPLICIT_REGION_ALIASES.get(normalizeLocationName(explicitRegionValue));
  if (explicitRegion) return explicitRegion;

  const district = [item.district, item.distrito, item.neighborhood, item.bairro].find(
    (value) => normalizeLocationName(value) !== "",
  );
