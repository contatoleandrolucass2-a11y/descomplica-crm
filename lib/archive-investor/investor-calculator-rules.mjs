...
function moneyValue(value) {
  ...
  const normalized = typeof clean === "string" && clean.includes(",") ? clean.replace(/\./g, "").replace(",", ".") : clean;
...
export function distributeSignalBalance(valueRealInput, entryValueInput) {
  ...
  const signalCount = missingInCents >= minimumInCents * 3 ? 3 : missingInCents >= minimumInCents * 2 ? 2 : 1;
...
function distributeScenarioSignals(valueReal, entryRate, targetEntryRate, signalCount) {
  ...
  return Array.from({ length: signalCount }, (_, index) =>
    (baseInCents + (index < remainder ? 1 : 0)) / 100,
  );
}
...
function standardScenario({ code, entryRate, targetEntryRate = entryRate, installmentLimit, signalLimit = 0, intermediaryLimit, valueReal, baseDate, completionDate, deadline, annualMode = false, income = 0 }) {
  const signalBaseDate = adjustCommercialDate(addDays(baseDate, 30));
  const signalValues = signalLimit > 0
  ...
  const standardMonthlyDates = buildCustomMonthlyDates(signalBaseDate, installmentLimit, signalLimit)
  ...
  const intermediaryDates = annualMode
    ? Array.from({ length: intermediaryLimit }, (_, index) => annualPaymentDate(baseDate, index))
  ...
  const intermediaryUnit = annualMode
    ? Math.min(valueReal * 0.05, income * 0.5)
    : valueReal * 0.05;
  ...
  const intermediaryTotalInCents = moneyInCents(intermediaryValues.reduce((total, value) => total + value, 0));
  const signalTotalInCents = moneyInCents(signalTotal);
  const balanceInCents = Math.max(0, valueRealInCents - entryInCents - signalTotalInCents - intermediaryTotalInCents);
  ...
  const lastInstallmentInCents = installments > 0
    ? balanceInCents - installmentBaseInCents * (installments - 1)
    : 0;
...
function standardAssociativeScenario({ code, signalLimit = 0, annualLimit = 0, valueReal, baseDate, completionDate, financing = 0, subsidy = 0, fgts = 0, housingCheck = 0, income = 0, installments = 84, tierId = "" }) {
  const tier = ASSOCIATIVE_APPROVAL_TIERS.find((item) => item.id === tierId) ?? ASSOCIATIVE_APPROVAL_TIERS[0];
  const safeInstallments = Math.max(1, Math.trunc(moneyValue(installments)) || 84);
  const resources = moneyValue(financing) + moneyValue(subsidy) + moneyValue(fgts) + moneyValue(housingCheck);
  const safeIncome = moneyValue(income);
  const monthlyCapacityRate = Math.max(0, Math.min(tier.commitmentRate, tier.annualIncomeLimitRate - 0.3));
  ...
  const incomeBalanceLimit = safeIncome > 0 ? safeIncome * monthlyCapacityRate * safeInstallments * 0.4 : 0;
  const proSolutoReductionTarget = Math.max(MINIMUM_SIGNAL_VALUE, valueReal - resources - tierBalanceLimit);
  const incomeReductionTarget = Math.max(MINIMUM_SIGNAL_VALUE, valueReal - resources - incomeBalanceLimit);
  const annualMaximum = Math.max(0, moneyValue(income) * 0.5);
  let annualReductionRemaining = annualLimit > 0
    ? Math.max(0, incomeReductionTarget - proSolutoReductionTarget)
    : 0;
  const annualInputs = Array.from({ length: ANNUAL_PAYMENT_COUNT }, (_, index) => {
    const dueDate = annualPaymentDate(baseDate, index);
    if (index >= annualLimit || dueDate > completionDate || annualMaximum <= 0 || annualReductionRemaining <= 0) return 0;
    ...
    annualReductionRemaining = Math.max(0, annualReductionRemaining - correctedAnnualValue(value, baseDate, dueDate));
    return value;
  });
  const allocatedAnnualTotal = annualInputs.reduce(
    (total, value, index) => total + correctedAnnualValue(value, baseDate, annualPaymentDate(baseDate, index)),
    0,
  );
  ...
  const equalPaymentInCents = Math.max(MINIMUM_SIGNAL_VALUE * 100, Math.ceil(reductionRemaining * 100 / paymentCount));
  ...
  const calculate = (scenarioFinancing) => calculateAssociativeLinear({
  ...
  const excess = Math.max(0, scenarioFinancing + moneyValue(subsidy) + moneyValue(fgts) + moneyValue(housingCheck) + entry + signalValues.reduce((total, value) => total + value, 0) - valueReal);
  ...
  const annualSchedule = linear.annualSchedule.filter((annual) => annual.amount > 0 && annual.valid);
  const firstInstallmentDate = dateFromIso(linear.firstInstallmentDate);
  const monthlyDates = firstInstallmentDate
    ? Array.from({ length: safeInstallments }, (_, index) => isoDate(addMonthsClamped(firstInstallmentDate, index)))
    : [];
  ...
  const maximumIncomeRate = safeIncome > 0 ? (highestPayment + safeIncome * 0.3) / safeIncome : Number.POSITIVE_INFINITY;
  const tierChecks = [
    ...
    { label: "% Máximo da renda por anual", ok: maximumIncomeRate <= tier.annualIncomeLimitRate + 1e-9 },
  ];
  const firstFailedTierCheck = tierChecks.find((check) => !check.ok);
  const available = valueReal > 0 && tier.id !== "not-eligible" && linear.ok && decreasing.ok && !firstFailedTierCheck;
  ...
  return {
    ...
    availabilityReason: available
      ...
      : tier.id === "not-eligible"
        ...
        : linear.errors[0] ?? decreasing.errors[0] ?? firstFailedTierCheck?.label ?? "Cenário fora da regra",
    approval: {
      tierId: tier.id,
      proSolutoRate,
      commitmentRate,
...
export function calculateInvestorFlow(input) {
  ...
  const signals = signalValues.map((value, index) => {
    ...
    else if (value < MINIMUM_SIGNAL_VALUE) reason = `Valor mínimo: R$ ${MINIMUM_SIGNAL_VALUE.toFixed(2).replace(".", ",")}`;
    else if (index > 0 && previousValue <= 0) reason = `Preencha o Sinal ${index} primeiro`;
    else if (index > 0 && value > previousValue) reason = `Sinal ${index + 1} não pode ser maior que o Sinal ${index}`;
  ...
  const signalGraceMonths = approvedSignals.reduce((latest, item) => Math.max(latest, item.index), 0);
  ...
  const intermediaryInputLimit = annualMode
    ? baseDate && completionDate
      ? Array.from({ length: ANNUAL_PAYMENT_COUNT }, (_, index) => annualPaymentDate(baseDate, index))
        .filter((date) => date >= baseDate && date <= completionDate).length
      : 0
    : desiredInstallments === 24 ? 4 : 3;
  const maxIntermediaries = annualMode ? intermediaryInputLimit : totalEntryRate < 0.1 ? 0 : intermediaryInputLimit;
  const monthsUntil = baseDate && completionDate ? fullMonthsUntil(baseDate, completionDate) : 0;
  const deadline = completionDate ? (annualMode ? completionDate : addMonthsClamped(completionDate, -3)) : null;
  const availableMonthlyDates = signalBaseDate && completionDate
    ? buildCustomMonthlyDates(signalBaseDate, entryLimit, signalGraceMonths)
      .filter((date) => annualMode || dateFromIso(date) <= completionDate)
    : [];
  const maxInstallments = availableMonthlyDates.length;
  const monthlyDates = signalBaseDate ? buildCustomMonthlyDates(signalBaseDate, desiredInstallments, signalGraceMonths) : [];
  const monthlyDateSet = new Set(monthlyDates);

  const intermediaries = intermediaryValues.map((value, index) => {
    const intermediaryDate = annualMode && baseDate
      ? annualPaymentDate(baseDate, index)
      : signalBaseDate
        ? addMonthsClamped(signalBaseDate, (index + 1) * 6 - 1)
        : baseDate ? adjustCommercialDate(addDays(baseDate, INTERMEDIARY_OFFSETS[index])) : null;
    if (!baseDate || value <= 0) {
      return { index: index + 1, value, correctedValue: 0, date: isoDate(intermediaryDate), rate: 0, approved: true, status: "Neutro", reason: annualMode ? "Não usada" : "Não usada" };
    }
    // Six-month cadence, anchored to the same 05/10/15 calendar as the monthly plan.
    const rate = valueReal > 0 ? value / valueReal : 0;
    let reason = "Dentro da regra";
    if (annualMode) {
      if (!intermediaryDate || intermediaryDate < baseDate) reason = "Vencimento anterior à data vigente";
      else if (!completionDate || intermediaryDate > completionDate) reason = "Anual após o término da obra";
    ...
    const correctedValue = approved && annualMode && intermediaryDate
  ...
  const customLinear = annualMode && baseDate && completionDate ? calculateAssociativeLinear({
    development: "Estoque selecionado",
  ...
  const installmentValue = customLinear
    ...
    : desiredInstallments > 0 && balance > 0 ? balance / desiredInstallments : 0;
  const customDecreasing = customLinear ? calculateAssociativeDecreasing({
  ...
  const customMonthlyDates = customFirstInstallmentDate
    ? Array.from({ length: desiredInstallments }, (_, index) => isoDate(addMonthsClamped(customFirstInstallmentDate, index)))
    : monthlyDates;

  const audit = [
    { id: "context", label: "Unidade, valor e data da obra válidos", ok: Boolean(input.selectedUnitId && propertyValue > 0 && completionDate && baseDate) },
    { id: "act", label: annualMode ? "Entrada mínima de R$ 150,00" : "Entrada mínima de 6%", ok: annualMode ? actValue >= MINIMUM_SIGNAL_VALUE : actRate >= 0.06 },
    { id: "entry", label: annualMode ? "Entrada total conforme WF-13" : "Entrada total mínima de 10%", ok: annualMode || totalEntryRate >= 0.1 },
    { id: "signals", label: invalidSignals[0]?.reason ?? "Sinais respeitam valor mínimo e ordem decrescente", ok: invalidSignals.length === 0 },
    { id: "construction", label: "Data da obra futura", ok: Boolean(baseDate && completionDate && completionDate > baseDate) },
    { id: "installments", label: installmentsInteger ? `Parcelas dentro do limite (${maxInstallments || 0})` : "Quantidade de parcelas deve ser inteira", ok: installmentsInteger && desiredInstallments > 0 && desiredInstallments <= maxInstallments },
    { id: "intermediary-count", label: annualMode ? "Até 5 anuais" : `Até ${maxIntermediaries} intermediárias`, ok: activeIntermediaries <= maxIntermediaries },
    { id: "intermediaries", label: annualMode ? "Anuais respeitam vencimento e término da obra" : "Intermediárias respeitam 5%, prazo e mensal", ok: invalidIntermediaries.length === 0 },
    { id: "associative-linear", label: customLinear?.errors[0] ?? "Motor WF-13 aplicado ao fluxo editável", ok: !annualMode || Boolean(customLinear?.ok) },
  ...
  const standardScenarios = baseDate && completionDate
    ? annualMode ? [
        standardAssociativeScenario({ code: "C1", valueReal, baseDate, completionDate, financing, subsidy, fgts, housingCheck, income, installments: desiredInstallments, tierId: input.approvalTierId }),
        standardAssociativeScenario({ code: "C2", signalLimit: 3, valueReal, baseDate, completionDate, financing, subsidy, fgts, housingCheck, income, installments: desiredInstallments, tierId: input.approvalTierId }),
        standardAssociativeScenario({ code: "C3", annualLimit: 5, valueReal, baseDate, completionDate, financing, subsidy, fgts, housingCheck, income, installments: desiredInstallments, tierId: input.approvalTierId }),
        standardAssociativeScenario({ code: "C4", signalLimit: 3, annualLimit: 5, valueReal, baseDate, completionDate, financing, subsidy, fgts, housingCheck, income, installments: desiredInstallments, tierId: input.approvalTierId }),
      ] : [
        standardScenario({ code: "C1", entryRate: 0.1, installmentLimit: 18, intermediaryLimit: 0, valueReal, baseDate, completionDate, deadline, annualMode, income }),
        standardScenario({ code: "C2", entryRate: 0.06, targetEntryRate: 0.1, installmentLimit: 18, signalLimit: 3, intermediaryLimit: 0, valueReal, baseDate, completionDate, deadline, annualMode, income }),
        standardScenario({ code: "C3", entryRate: 0.06, targetEntryRate: 0.1, installmentLimit: 18, signalLimit: 3, intermediaryLimit: annualMode ? 5 : 3, valueReal, baseDate, completionDate, deadline, annualMode, income }),
        standardScenario({ code: "C4", entryRate: 0.1, installmentLimit: 18, intermediaryLimit: annualMode ? 5 : 3, valueReal, baseDate, completionDate, deadline, annualMode, income }),
        standardScenario({ code: "C5", entryRate: 0.2, installmentLimit: 24, intermediaryLimit: 0, valueReal, baseDate, completionDate, deadline, annualMode, income }),
        standardScenario({ code: "C6", entryRate: 0.17, targetEntryRate: 0.2, installmentLimit: 24, signalLimit: 3, intermediaryLimit: 0, valueReal, baseDate, completionDate, deadline, annualMode, income }),
        standardScenario({ code: "C7", entryRate: 0.17, targetEntryRate: 0.2, installmentLimit: 24, signalLimit: 3, intermediaryLimit: annualMode ? 5 : 4, valueReal, baseDate, completionDate, deadline, annualMode, income }),
        standardScenario({ code: "C8", entryRate: 0.2, installmentLimit: 24, intermediaryLimit: annualMode ? 5 : 4, valueReal, baseDate, completionDate, deadline, annualMode, income }),
      ]
    : [];
