...
export function resolveAssociativeAppraisal(reportedAppraisal, appraisalFallback) {
  const officialAppraisal = finiteNumber(reportedAppraisal);
  if (officialAppraisal > 0) return officialAppraisal;

  const fallbackAppraisal = finiteNumber(appraisalFallback);
  return fallbackAppraisal > 0 ? fallbackAppraisal : 0;
}
...
export function calculateAssociativeDocumentationView(input) {
  ...
  const appraisalValue = resolveAssociativeAppraisal(reportedAppraisal, appraisalOverride);
