...
export function isDirectTableProposalPresetComplete(optionId, preset) {
  const option = DIRECT_TABLE_PROPOSAL_OPTIONS.find((item) => item.id === optionId);
  if (!option || !preset) return false;

  const positiveSignals = Array.isArray(preset.signals)
    ? preset.signals.filter((value) => moneyValue(value) > 0).length
    : 0;
  const positiveIntermediaries = Array.isArray(preset.intermediaries)
    ? preset.intermediaries.filter((value) => moneyValue(value) > 0).length
    : 0;

  if (option.withSignals && (preset.signalFieldCount !== 3 || positiveSignals !== 3)) {
    return false;
  }
  if (
    option.withIntermediary &&
    (preset.intermediaryFieldCount < 1 || positiveIntermediaries < 1)
  ) {
    return false;
  }
  return true;
...
function buildIntermediaryPreset(option, valueReal, dates, policy) {
  ...
  if (count === maxAtFivePercent) {
    const targetTotal = resolveNominalBlocks(valueReal, policy).preKeysBudget;
...
function resolveNominalBlocks(valueReal, policy) {
  const minimumEntryValue = roundMoney(valueReal * DIRECT_TABLE_PARAMETERS.actMinimumRate);
  const postKeysBalance = roundMoney(valueReal * policy.postKeysRate);
  const preKeysBudget = roundMoney(Math.max(0, valueReal - minimumEntryValue - postKeysBalance));
  return { minimumEntryValue, postKeysBalance, preKeysBudget };
}
...
export function calculateDirectTableFileFlow(input) {
  ...
  const salePrice = roundMoney(moneyValue(input.salePrice));
  const discount = input.discountAuthorized ? roundMoney(moneyValue(input.discount)) : 0;
  const valueReal = roundMoney(Math.max(0, salePrice - discount));
  const policy = resolveDirectTablePolicy(input.plant);
  const income = roundMoney(moneyValue(input.income));
  const actValue = roundMoney(moneyValue(input.entryValue));
  const actRate = valueReal > 0 ? actValue / valueReal : 0;
  const signalValues = Array.from({ length: 3 }, (_, index) =>
    roundMoney(moneyValue(input.signals?.[index])),
  );
  const intermediaryValues = INTERMEDIARY_OFFSETS.map((_, index) =>
    roundMoney(moneyValue(input.intermediaries?.[index])),
  );

  let previousSignalDate = baseDate;
  let previousSignalApproved = true;
  const signals = signalValues.map((value, index) => {
    ...
    else if (index > 0 && !previousSignalApproved) reason = `Corrija o Sinal ${index} primeiro`;
    else if (value > previousValue)
      reason =
        index === 0
          ? "Sinal 1 não pode ser maior que o ato"
          : `Sinal ${index + 1} não pode ser maior que o Sinal ${index}`;
    else if (!completionDate || !signalDate || signalDate > completionDate)
      reason = "Sinal após a data de entrega";
    const approved = value <= 0 || reason === "Dentro da regra";
    previousSignalApproved = value > 0 && approved;
    return {
      index: index + 1,
      value,
  ...
  const signalTotal = roundMoney(approvedSignals.reduce((total, item) => total + item.value, 0));
  const totalEntryValue = roundMoney(actValue + signalTotal);
  const totalEntryRate = valueReal > 0 ? totalEntryValue / valueReal : 0;
  const nominalBlocks = resolveNominalBlocks(valueReal, policy);
  const minimumActValue = roundMoney(valueReal * DIRECT_TABLE_PARAMETERS.scenarioTwoActRate);
  const minimumEntryValue = nominalBlocks.minimumEntryValue;
  const postKeysBalance = nominalBlocks.postKeysBalance;
  const maximumEntryValue = roundMoney(valueReal - postKeysBalance);
  const entryMeetsMinimum = totalEntryValue >= minimumEntryValue;
  const entryWithinMaximum = totalEntryValue <= maximumEntryValue;
  const entryExcess = entryMeetsMinimum ? roundMoney(totalEntryValue - minimumEntryValue) : 0;
  const preKeysBudget = roundMoney(Math.max(0, nominalBlocks.preKeysBudget - entryExcess));
  const lastEntryDate =
  ...
  const intermediaryBudget = preKeysBudget;
  ...
  const intermediaries = intermediaryValues.map((value, index) => {
    ...
    if (value <= 0) reason = "Não usada";
    else if (!entryMeetsMinimum) reason = "Entrada total menor que 10%";
      ...
      reason =
        entryExcess > 0
          ? "Intermediárias acima do saldo pré-chaves após a entrada"
          : `Intermediárias acima do saldo pré-chaves de ${policy.preKeysRate * 100}%`;
  ...
  const validIntermediaryTotal = roundMoney(
    intermediaries
      .filter((item) => item.value > 0 && item.approved)
      .reduce((total, item) => total + item.value, 0),
  );
  const preKeysBalance = roundMoney(Math.max(0, preKeysBudget - validIntermediaryTotal));
  const preKeysSchedule = buildDirectTablePreKeysSchedule(
    preKeysBalance,
    preKeysInstallments,
    isoDate(firstPreKeysDate),
  );
  const preKeysPayment = preKeysSchedule[0]?.payment ?? 0;
  const lastPreKeysPayment = preKeysSchedule.at(-1)?.payment ?? 0;
  const lastPreKeysDate =
    preKeysInstallments > 0 ? addMonthsClamped(firstPreKeysDate, preKeysInstallments - 1) : null;
  const postKeysAnchorDate = lastPreKeysDate ?? completionDate;
  const firstPostKeysDate = postKeysAnchorDate ? highestCommercialDate(postKeysAnchorDate) : null;
  ...
  const postKeysPayment =
    buildDirectTableAmortizationSchedule(
      postKeysBalance,
      isoDate(firstPostKeysDate),
      policy.postKeysInstallments,
    )[0]?.totalPayment ?? 0;
  ...
  const nominalDistribution = roundMoney(
    totalEntryValue + validIntermediaryTotal + preKeysBalance + postKeysBalance,
  );
  ...
  const audit = [
    ...
    },
    {
      ...
      ok: actValue >= minimumActValue,
    },
    {
      id: "entry",
      label: "Ato e sinais totalizam pelo menos 10%",
      ok: entryMeetsMinimum,
    },
    {
      id: "entry-maximum",
      label: `Ato e sinais preservam ${policy.postKeysRate * 100}% para o pós-chaves`,
      ok: entryWithinMaximum,
    },
    {
      id: "signals",
    ...
    {
      id: "distribution",
      label: `Entrada + saldo pré-chaves + ${policy.postKeysRate * 100}% pós-chaves totalizam o valor do imóvel`,
      ok: valueReal > 0 && nominalDistribution === valueReal,
    },
    {
      id: "pre-keys",
      label:
        preKeysBalance === 0
          ...
          : preKeysInstallments > 0
            ? `${preKeysInstallments} mensais pré-chaves calculadas`
            : "Prazo insuficiente para mensais pré-chaves",
      ok: preKeysBalance === 0 || preKeysInstallments > 0,
    },
    {
      id: "intermediaries",
      label:
  ...
  const proposalReady = audit
    .filter((item) => item.id !== "income" && item.id !== "credit")
    .every((item) => item.ok);
  const status =
    income <= 0
      ? "PENDENTE"
  ...
  return {
    ...
    custom: {
      ...
      maximumEntryValue,
      entryExcess,
      basePreKeysBudget: nominalBlocks.preKeysBudget,
      preKeysBudget,
      balance: preKeysBalance,
      installmentValue: preKeysPayment,
      lastInstallmentValue: lastPreKeysPayment,
      installmentValues: preKeysSchedule.map((item) => item.payment),
