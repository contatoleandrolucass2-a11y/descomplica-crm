import { calculateDirectTable, DIRECT_TABLE_PARAMETERS, resolveDirectTablePolicy } from "./direct-table-rules.mjs";
export { buildDirectTableAmortizationSchedule, buildDirectTablePreKeysSchedule } from "./direct-table-rules.mjs";

const INTERMEDIARY_OFFSETS = [180, 360, 540, 720, 900, 1080, 1260, 1440];

export const DIRECT_TABLE_PROPOSAL_OPTIONS = Object.freeze([
  Object.freeze({ id: "without-signal-without-intermediary", label: "Opção sem sinal e sem intermediária", title: "Pagamento simples", entrySummary: "Ato de 10%", signalSummary: "Sem sinais", intermediarySummary: "Sem intermediária", withSignals: false, withIntermediary: false }),
  Object.freeze({ id: "with-signal-without-intermediary", label: "Opção com sinal e sem intermediária", title: "Entrada distribuída", entrySummary: "Ato de 6%", signalSummary: "3 sinais somam 4%", intermediarySummary: "Sem intermediária", withSignals: true, withIntermediary: false }),
  Object.freeze({ id: "without-signal-with-intermediary", label: "Opção sem sinal e com intermediária", title: "Parcela reduzida", entrySummary: "Ato de 10%", signalSummary: "Sem sinais", intermediarySummary: "Intermediárias de 5% conforme a entrega", withSignals: false, withIntermediary: true }),
  Object.freeze({ id: "with-signal-with-intermediary", label: "Opção com sinal e com intermediária", title: "Maior flexibilidade", entrySummary: "Ato de 6%", signalSummary: "3 sinais somam 4%", intermediarySummary: "Intermediárias de 5% conforme a entrega", withSignals: true, withIntermediary: true }),
]);
...
export function buildDirectTableProposalPreset(optionId, valueRealInput, dates = {}) {
  ...
  if (option.withSignals) {
    ...
    const currentEntry = roundMoney(entryValue + signals.reduce((total, value) => total + value, 0));
...
function buildIntermediaryPreset(option, valueReal, dates, policy) {
  ...
  const monthlyDateSet = new Set(firstPreKeysDate
    ? Array.from({ length: preKeysInstallments }, (_, index) => isoDate(addMonthsClamped(firstPreKeysDate, index)))
    : []);
  const deadline = addMonthsClamped(completionDate, -3);
  const eligibleDates = eligibleIntermediaryDates(baseDate, firstPreKeysDate, deadline, monthlyDateSet);
  const maxAtFivePercent = Math.floor((policy.preKeysRate + Number.EPSILON)
    / DIRECT_TABLE_PARAMETERS.intermediaryMaxRate);
...
function eligibleIntermediaryDates(baseDate, firstPreKeysDate, deadline, monthlyDateSet) {
  ...
  return INTERMEDIARY_OFFSETS
    .map((offset) => fixedDayFromOffset(baseDate, offset, firstPreKeysDate.getUTCDate()))
    .filter((date) => date >= firstPreKeysDate && date <= deadline && monthlyDateSet.has(isoDate(date)));
}
...
function highestCommercialDate(baseDate) {
  ...
  for (let monthOffset = 0; monthOffset <= 2; monthOffset += 1) {
    const month = addMonthsClamped(new Date(Date.UTC(baseDate.getUTCFullYear(), baseDate.getUTCMonth(), 1, 12)), monthOffset);
...
function mapStandardScenario(result, scenario, income) {
  ...
  return {
    ...
    installmentValues: Array.from({ length: scenario.preKeysInstallments }, () => scenario.preKeysPayment),
    firstInstallmentDate: scenario.firstPreKeysDate,
    monthlyDates: Array.from({ length: scenario.preKeysInstallments }, (_, index) => isoDate(addMonthsClamped(parseIsoDate(scenario.firstPreKeysDate), index))),
    ...
    creditApproved: income > 0 && scenario.postKeysPayment / income <= DIRECT_TABLE_PARAMETERS.creditCommitmentLimit,
...
export function calculateDirectTableFileFlow(input) {
  ...
  const intermediaryValues = INTERMEDIARY_OFFSETS.map((_, index) => moneyValue(input.intermediaries?.[index]));
  ...
  const signals = signalValues.map((value, index) => {
    ...
    else if (previousValue <= 0) reason = index === 0 ? "Preencha o ato primeiro" : `Preencha o Sinal ${index} primeiro`;
    else if (value > previousValue) reason = index === 0 ? "Sinal 1 não pode ser maior que o ato" : `Sinal ${index + 1} não pode ser maior que o Sinal ${index}`;
  ...
  const lastEntryDate = approvedSignals.length > 0
    ? parseIsoDate(approvedSignals.at(-1).date)
    : baseDate;
  ...
  const intermediaryInputLimit = eligibleIntermediaryDates(baseDate, firstPreKeysDate, deadline, monthlyDateSet).length;
  ...
  const intermediaries = intermediaryValues.map((value, index) => {
    const intermediaryDate = baseDate && firstPreKeysDate
    ...
    const maximumValue = Math.round(valueReal * DIRECT_TABLE_PARAMETERS.intermediaryMaxRate * 100) / 100;
    let reason = "Dentro da regra";
    if (value <= 0) reason = "Não usada";
    else if (totalEntryRate < DIRECT_TABLE_PARAMETERS.actMinimumRate) reason = "Entrada total menor que 10%";
    else if (value > maximumValue) reason = "Intermediária acima de 5%";
    else if (roundMoney(approvedIntermediaryTotal + value) > intermediaryBudget) reason = `Intermediárias acima do saldo pré-chaves de ${policy.preKeysRate * 100}%`;
    else if (!deadline || intermediaryDate > deadline) reason = "Fora do prazo da obra";
    else if (!firstPreKeysDate || intermediaryDate < firstPreKeysDate) reason = "Anterior à primeira mensal pré-chaves";
    else if (!monthlyDateSet.has(isoDate(intermediaryDate))) reason = "Sem mensal na mesma data";
    const approved = value <= 0 || reason === "Dentro da regra";
    if (value > 0 && approved) approvedIntermediaryTotal = roundMoney(approvedIntermediaryTotal + value);
    return {
      index: index + 1,
      value,
  ...
  const lastPreKeysDate = preKeysInstallments > 0 ? addMonthsClamped(firstPreKeysDate, preKeysInstallments - 1) : null;
  ...
  const standardScenarios = standardResult.scenarios?.map((scenario) => mapStandardScenario(standardResult, scenario, income)) ?? [];
  ...
  const audit = [
    { id: "context", label: "Unidade, valor e data da obra válidos", ok: Boolean(input.selectedUnitId && salePrice > 0 && completionDate && baseDate && completionDate > baseDate) },
    { id: "act", label: "Ato mínimo de 6%", ok: actValue >= roundMoney(valueReal * DIRECT_TABLE_PARAMETERS.scenarioTwoActRate) },
    { id: "entry", label: "Ato e sinais totalizam pelo menos 10%", ok: totalEntryRate >= DIRECT_TABLE_PARAMETERS.actMinimumRate },
    { id: "signals", label: invalidSignals[0]?.reason ?? "Sinais respeitam a ordem e a cascata", ok: invalidSignals.length === 0 },
    { id: "distribution", label: `Distribuição 10% + ${policy.preKeysRate * 100}% + ${policy.postKeysRate * 100}%`, ok: valueReal > 0 },
    { id: "pre-keys", label: `${preKeysInstallments} mensais pré-chaves calculadas`, ok: preKeysInstallments > 0 && preKeysBalance >= 0 },
    { id: "intermediaries", label: invalidIntermediaries[0]?.reason ?? "Até 8 intermediárias respeitam 5%, prazo e mensal", ok: invalidIntermediaries.length === 0 },
    { id: "post-keys", label: `Pós-chaves em ${policy.postKeysInstallments} parcelas com juros, MIP e DFI`, ok: postKeysPayment > 0 },
    { id: "income", label: "Renda mensal informada", ok: income > 0 },
    { id: "credit", label: income > 0 ? `Comprometimento de renda até 40%` : "Informe a renda para validar o crédito", ok: creditApproved },
  ];
  ...
  return {
    ...
    custom: {
      ...
      validIntermediaryCount: intermediaries.filter((item) => item.value > 0 && item.approved).length,
      intermediaries,
