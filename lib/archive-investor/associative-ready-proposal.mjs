...
const SEPARATED_COMMISSION_RANKINGS = Object.freeze({
  gold: Object.freeze({ label: "Ouro", rate: 0.045 }),
  silver: Object.freeze({ label: "Prata", rate: 0.04 }),
  bronze: Object.freeze({ label: "Bronze", rate: 0.035 }),
});

const SEPARATED_COMMISSION_ENTRY_RATE = 0.06;
...
function roundUpMoney(value) {
  ...
  return (scaled < 0
    ...
    : Math.ceil(scaled - tolerance)) / 100;
}
...
function roundDownMoney(value) {
  ...
  return (scaled < 0
    ...
    : Math.floor(scaled + tolerance)) / 100;
}
...
export function buildAssociativeReadyProposalResponseRows(calculation = {}) {
  ...
  const separatedProposal = separatedCommission?.eligible
    ? separatedCommission.proposal
    : null;
  ...
  const row = (key, label, operator, value, options = {}) => {
    ...
    return {
      ...
      separatedCommissionValue:
        responseRow.separatedCommissionValue > 0
          ? responseRow.separatedCommissionValue
          : null,
    };
  };
  ...
  return [
    ...
    (responseRow) =>
      ...
      (responseRow.separatedCommissionValue !== null &&
        responseRow.separatedCommissionValue > 0),
  );
}
...
export function buildAssociativeReadyProposal(input = {}) {
  ...
  const financingCapacity = roundDownMoney(
    Math.min(appraisalLimit, financingTableLimit),
  );
  ...
  const commissionEntryThreshold = roundMoney(
    source.netSaleValue * SEPARATED_COMMISSION_ENTRY_RATE,
  );
  ...
  const commissionValue = commissionRanking
    ...
    : 0;
  const awardBase = roundMoney(
    Math.max(0, source.cashBackSlack - source.sourceDiscount),
  );
  ...
  const separatedNetSaleValue = roundMoney(
    Math.max(0, source.netSaleValue - totalRemuneration),
  );
  ...
  const separatedContractValue = roundMoney(
    Math.max(
      ...
      Math.min(
        separatedContractMinimum,
        source.appraisal,
        separatedContractMaximum,
      ),
    ),
  );
  ...
  const separatedProposalDiscount = roundMoney(
    Math.max(
      0,
      source.grossSaleValue - totalRemuneration - separatedContractValue,
    ),
  );
  const separatedResourceTotal = roundMoney(
    separatedFinancing + source.subsidy + source.fgts + source.housingCheck,
  );
  const separatedBalanceAfterResources = roundMoney(
    separatedNetSaleValue - separatedResourceTotal,
  );
  const separatedMonthlyBalance = roundMoney(
    separatedBalanceAfterResources - source.entry - signalTotal - annualTotal,
  );
  const separatedAverageInstallment = roundDownMoney(
    separatedMonthlyBalance / source.installments,
  );
  ...
  return {
    ...
    separatedCommission: {
      ...
      entryRate: SEPARATED_COMMISSION_ENTRY_RATE,
