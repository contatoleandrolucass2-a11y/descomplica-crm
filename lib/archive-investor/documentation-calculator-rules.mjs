import { evaluateFinancingModality, moneyToCents, MCMV_PROPERTY_LIMIT_CENTS } from "./financing-modality-rules.mjs";
...
export const REGISTRATION_TABLE = Object.freeze([
  [0.01, 250.89], [2222.01, 402.59], [5551.01, 722.25], [9253.01, 1071.65],
  [18510.01, 1302.87], [37020.01, 1452.97], [111060.01, 1854.49], [185100.01, 2255.22],
  [222120.01, 2455.19], [259140.01, 2656.29], [296160.01, 2800.25], [333180.01, 2873.24],
  [370200.01, 3203.68], [740400.01, 3751.85], [1110600.01, 4319.28], [1480800.01, 4886.77],
  [1851000.01, 5180.15], [2221200.01, 6647.08], [3702000.01, 9287.49], [5553000.01, 12221.3],
  [7404000.01, 15155.11], [9255000.01, 18088.94], [11106000.01, 21022.73], [12957000.01, 23956.56],
  [14808000.01, 26890.36], [16659000.01, 29824.18], [18510000.01, 34224.89], [22212000.01, 40092.51],
  [25914000.01, 45960.13], [29616000.01, 51827.75], [33318000.01, 57695.39], [37020000.01, 63563.01],
  [40722000.01, 69430.64], [44424000.01, 75298.27], [48126000.01, 81165.89], [51828000.01, 87033.51],
  [55530000.01, 95834.95], [62934000.01, 107570.19], [70338000.01, 119305.45], [77742000.01, 131040.7],
  [85146000.01, 142775.96], [92550000.01, 154511.21], [99954000.01, 166246.46], [107358000.01, 177981.7],
  [114762000.01, 189716.95], [122166000.01, 201452.21], [129570000.01, 213187.45], [136974000, 225606.27],
].map((row) => Object.freeze(row)));

function normalizeText(value) {
  return typeof value === "string"
    ? value.trim().replace(/\s+/g, " ").normalize("NFD").replace(/[\u0300-\u036f]/g, "").toUpperCase()
    : "";
}
...
export function buildDocumentationInstallmentSchedule({ firstInstallmentDate, installments, installmentValue }) {
  const firstDate = parseDateValue(firstInstallmentDate);
  const count = Number(installments);
  const value = parseFinancialValue(installmentValue);
  if (!firstDate || !Number.isInteger(count) || count < 1 || !(Number.isFinite(value) && value > 0)) return [];
...
function highestValidDueDate(baseDate) {
  ...
  let selected = null;
  for (let cursor = new Date(Date.UTC(baseDate.getUTCFullYear(), baseDate.getUTCMonth(), 1)); cursor <= limit; cursor.setUTCMonth(cursor.getUTCMonth() + 1)) {
    for (const day of OFFICIAL_PARAMETERS.validDueDays) {
      ...
      if (candidate >= baseDate && candidate <= limit && (!selected || candidate > selected)) selected = candidate;
    }
  }
  return selected;
}
...
function chooseFirstInstallmentDate(baseDate, requestedValue) {
  ...
  const limit = addUtcDays(baseDate, OFFICIAL_PARAMETERS.firstInstallmentWindowDays);
  const requestedIsValid = requested
    && requested >= baseDate
    && requested <= limit
...
function calculateItbi({ salePrice, financing, effectiveModality, firstProperty }) {
  if (firstProperty === "SIM" && effectiveModality === "MCMV" && salePrice <= OFFICIAL_PARAMETERS.itbiExemptionLimit) {
  ...
  if (salePrice <= OFFICIAL_PARAMETERS.progressiveItbiLimit) {
    ...
    const value = Math.max(0, (reducedBase * OFFICIAL_PARAMETERS.reducedItbiRate) + ((salePrice - reducedBase) * OFFICIAL_PARAMETERS.fullItbiRate));
    return { value, rule: "Progressivo: 0,5% sobre a base reduzida do financiamento e 3% sobre o excedente da venda." };
  }
  return { value: salePrice * OFFICIAL_PARAMETERS.fullItbiRate, rule: "Integral: 3% sobre o valor da venda." };
}
...
function registrationTableIsValid() {
  return REGISTRATION_TABLE.length === 48 && REGISTRATION_TABLE.every(([start, fee], index) => (
...
export function calculateDocumentation(input) {
  ...
  const effectiveModality = modalityDecision.effectiveModality ?? (firstProperty === "NAO" ? "SBPE" : informedModality);
  ...
  const mcmvPropertyRange = Number.isFinite(salePrice) && salePrice > 0 ? propertyRange(salePrice) : 0;
  const mcmvEligible = effectiveModality !== "MCMV"
    || (
      income <= OFFICIAL_PARAMETERS.mcmvIncomeLimit
      && salePrice <= OFFICIAL_PARAMETERS.mcmvPropertyLimit
    );
  const normalizations = [];
  if (typeof input.businessUnit === "string" && input.businessUnit !== businessUnit) normalizations.push("unidade_negocio");
  if (typeof input.modality === "string" && normalizeText(input.modality) !== informedModality) normalizations.push("modalidade");
  if (typeof input.firstProperty === "boolean" || (typeof input.firstProperty === "string" && input.firstProperty !== firstProperty)) normalizations.push("primeiro_imovel");
  const errors = [];
  ...
  if (!(Number.isFinite(salePrice) && salePrice > 0)) errors.push("Valor da venda deve ser numérico e maior que zero.");
  if (!(Number.isFinite(appraisalValue) && appraisalValue > 0)) errors.push("Avaliação bancária deve ser numérica e maior que zero.");
  if (!(Number.isFinite(financing) && financing > 0)) errors.push("Valor do financiamento deve ser numérico e maior que zero.");
  if (input.income !== "" && input.income != null && !Number.isFinite(income)) errors.push("Renda deve ser numérica.");
  if (effectiveModality === "MCMV" && !(income > 0)) errors.push("Informe a renda para validar cenário MCMV.");
  if (effectiveModality === "MCMV" && income > 0 && !mcmvEligible) errors.push("Renda ou valor do imóvel incompatível com o enquadramento MCMV.");
  ...
  const financingRate = effectiveModality === "MCMV"
    ? OFFICIAL_PARAMETERS.mcmvFinancingLimit
    : OFFICIAL_PARAMETERS.sbpeFinancingLimit;
  const appraisalFinancingLimit = Number.isFinite(appraisalValue) && appraisalValue > 0 ? roundMoney(appraisalValue * financingRate) : 0;
  const maximumFinancing = appraisalFinancingLimit > 0 && salePrice > 0
    ? Math.min(appraisalFinancingLimit, roundMoney(salePrice))
    : appraisalFinancingLimit;
  if (financing > salePrice && salePrice > 0) errors.push("Financiamento não pode superar o valor da venda.");
  if (financing > appraisalFinancingLimit && appraisalFinancingLimit > 0) errors.push(`Financiamento supera teto de ${Math.round(financingRate * 100)}% da avaliação bancária.`);

  const purchaseRegistrationBase = tableIsValid && salePrice > 0 ? lookupRegistration(salePrice) : null;
  const lienRegistrationBase = tableIsValid && financing > 0 ? lookupRegistration(financing) : null;
  if (salePrice > 0 && purchaseRegistrationBase == null) errors.push("Faixa de registro da compra e venda não localizada.");
  if (financing > 0 && lienRegistrationBase == null) errors.push("Faixa de registro da alienação não localizada.");

  const audit = [
    { label: "UDN", ok: ['Direcional', 'Riva'].includes(businessUnit) },
    { label: "Modalidade efetiva", ok: ['MCMV', 'SBPE'].includes(effectiveModality) },
    { label: "Valores financeiros", ok: Number.isFinite(salePrice) && salePrice > 0 && Number.isFinite(appraisalValue) && appraisalValue > 0 && Number.isFinite(financing) && financing > 0 },
    { label: "Teto do financiamento", ok: financing > 0 && financing <= maximumFinancing && financing <= salePrice },
    { label: "Enquadramento MCMV", ok: mcmvEligible },
    { label: "Data-base", ok: Boolean(baseDate) },
    { label: "Tabela local de registro", ok: tableIsValid && purchaseRegistrationBase != null && lienRegistrationBase != null },
  ];

  if (errors.length > 0) {
    return { ok: false, errors, businessUnit, informedModality, effectiveModality, modalityDecision, maximumFinancing, financingRate, incomeRange: mcmvIncomeRange, propertyRange: mcmvPropertyRange, normalizations, audit };
  }
  ...
  const purchaseRegistration = roundMoney(purchaseRegistrationBase * (firstPropertyBenefits ? OFFICIAL_PARAMETERS.firstPropertyPurchaseRegistrationFactor : 1));
  const lienRegistration = roundMoney(lienRegistrationBase * (firstPropertyBenefits ? OFFICIAL_PARAMETERS.firstPropertyLienRegistrationFactor : 1));
  const totalRegistration = roundMoney(purchaseRegistration + lienRegistration);
  const totalCash = roundMoney(OFFICIAL_PARAMETERS.dispatchFee + itbiValue + totalRegistration + OFFICIAL_PARAMETERS.caixaInsurance);
  const installments = businessUnit === "Direcional" ? OFFICIAL_PARAMETERS.direcionalInstallments : OFFICIAL_PARAMETERS.rivaInstallments;
