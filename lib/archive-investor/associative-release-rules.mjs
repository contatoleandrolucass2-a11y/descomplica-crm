...
export function validateAssociativeSignalSequence(signals = []) {
  const values = signals
    .slice(0, 3)
    .map((signal) => Math.max(0, roundMoney(signal?.value ?? signal ?? 0)));
...
function signalReleaseDate(signals, releaseThreshold) {
  let outstanding = roundMoney(
    signals.reduce((total, signal) => total + Math.max(0, Number(signal.value) || 0), 0),
  );
  const scheduled = signals
    .map((signal) => ({
      value: Math.max(0, roundMoney(signal.value)),
...
export function calculateAssociativeReleaseStatus({
  ...
  const signalTotal = roundMoney(
    normalizedSignals.reduce((total, signal) => total + signal.value, 0),
  );
  ...
  const repasseReleaseDate = signalBlocked
    ? signalReleaseDate(normalizedSignals, signalReleaseThreshold)
    : null;
  const repasseReleased =
    !signalBlocked || Boolean(reference && repasseReleaseDate && reference >= repasseReleaseDate);
  const entryUnlockDate = entry >= commissionEntryThreshold ? addCalendarDays(entryDate, 7) : null;
  const commissionReleasedByEntry = Boolean(
    entryUnlockDate && reference && reference >= entryUnlockDate,
  );
