...
const MIP_MCMV = [[18, 25, 0.0082], [26, 30, 0.0085], [31, 35, 0.0108], [36, 40, 0.0144], [41, 45, 0.0244], [46, 50, 0.0359], [51, 55, 0.0645], [56, 60, 0.0764], [61, 65, 0.1296], [66, 70, 0.2005], [71, 75, 0.3729], [76, 99, 0.4566]];
const MIP_SBPE = [[18, 25, 0.0093], [26, 30, 0.0096], [31, 35, 0.0116], [36, 40, 0.0154], [41, 45, 0.0252], [46, 50, 0.0386], [51, 55, 0.0676], [56, 60, 0.1533], [61, 65, 0.2731], [66, 70, 0.3259], [71, 75, 0.4894], [76, 99, 0.5312]];

const FDR_BY_STATE = {
  DF: 7.17, GO: -0.68, MS: -3.49, MT: -0.13,
  AL: -8.14, BA: -5.03, CE: -7.87, MA: 0.69, PB: -6.78, PE: -5.31, PI: -8.13, RN: 0.58, SE: -6.18,
  AC: 3.44, AM: 0.56, AP: 10, PA: 5.45, RO: -10, RR: -4.83, TO: -3.17,
  ES: -5.1, MG: -5.68, RJ: -1.78, SP: -5.68,
  PR: -5.13, RS: -1.09, SC: 4.58,
};

const NORTH = new Set(["AC", "AM", "AP", "PA", "RO", "RR", "TO"]);
const NORTH_NORTHEAST = new Set(["AC", "AL", "AM", "AP", "BA", "CE", "MA", "PA", "PB", "PE", "PI", "RN", "RO", "RR", "SE", "TO"]);
...
export function getMaximumTerm({ birthDate, product = "mcmv", system = "price", inConstruction = false, asOf }) {
  const reference = asOf ? new Date(asOf) : new Date();
  const birth = birthDate ? new Date(`${birthDate}T00:00:00.000Z`) : null;
  const ageLimit = birth && !Number.isNaN(birth.getTime())
...
export function getHousingRate({ income, state = "SP", propertyValue = 0, product = "mcmv", hasFgts36 = false, previousSubsidy = false }) {
...
export function calculateSubsidy({ income, propertyValue, state = "SP", cityLimit = 0, populationFactor = 1, socialFactor = true, previousSubsidy = false }) {
  if (previousSubsidy || income > 4000 || income > 3700 || !income || !propertyValue || populationFactor <= 0) return 0;
  const maximumDiscount = 50000;
  const minimumDiscount = 1900;
  const maximumIncome = 1750;
  const minimumIncome = 3700;
  const b = (2 * maximumDiscount * (minimumDiscount / maximumDiscount - 1)) / (minimumIncome - maximumIncome);
  const a = -b / (2 * (minimumIncome - maximumIncome));
  const incomeFactor = Math.max(0, a * (income - maximumIncome) ** 2 + b * (income - maximumIncome) + maximumDiscount);
...
export function calculateHousingSimulation(input) {
  ...
  const subsidy = product === "mcmv" ? calculateSubsidy({
    income,
    propertyValue,
    state: input.state,
  ...
  const monthlyFactor = system === "price"
    ? calculatePrice(1, rate, term)
    : calculateSacFirst(1, rate, term);
  ...
  if (approvedPayment > maximumPaymentByIncome) {
    return {
      ok: false,
      errors: [`A prestação informada ultrapassa ${incomeLimitPercent}% da renda. O teto é ${maximumPaymentByIncome.toFixed(2)}.`],
    };
  }
  ...
  const incomeFinancing = availableForFinancing > 0 ? availableForFinancing / (monthlyFactor + mip) : 0;
  const financing = Math.max(0, Math.min(requestedFinancing, quotaFinancing, incomeFinancing));
  const principalPayment = system === "price"
...
export function calculateExtraAmortization({ balance, annualRate, remainingTerm, system = "price", contribution = 0 }) {
  ...
  if (!principal || !rate || !term) return { ok: false, errors: ["Informe saldo, taxa e prazo restantes."] };
  const originalInterest = totalInterest(principal, rate, term, system);
  const originalPayment = system === "sac" ? calculateSacFirst(principal, rate, term) : calculatePrice(principal, rate, term);
  const monthlyRate = rate / 100 / 12;
  ...
  const newPayment = system === "sac" ? calculateSacFirst(newBalance, rate, term) : calculatePrice(newBalance, rate, term);
  return {
    ok: true,
    originalPayment,
    newPayment,
...
export function calculateFgtsTime(periods, asOf = new Date()) {
  const normalized = periods.map((period) => {
    ...
    if (!period.start || Number.isNaN(start.getTime()) || Number.isNaN(end.getTime()) || end <= start) return null;
    return { start, end, days: Math.round((end - start) / 86400000) };
  }).filter(Boolean);
  const overlap = normalized.some((period, index) => normalized.some((other, otherIndex) => index < otherIndex && period.start < other.end && other.start < period.end));
...
export function estimateConstructionInterest({ financing, annualRate, months, currentProgress = 0, trAnnual = 2, mipMonthly = 0, operatingFee = 25 }) {
  const principal = positive(financing);
  const term = Math.floor(positive(months));
  if (!principal || !term) return { ok: false, errors: ["Informe financiamento e meses restantes da obra."] };
