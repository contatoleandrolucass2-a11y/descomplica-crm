...
export function matchesInvestorFilters(item, filters, ignoredDimension = null) {
  if (ignoredDimension !== "businessUnit" && filterValue(filters, "businessUnit") !== ALL_VALUES.businessUnit && item.businessUnit !== filterValue(filters, "businessUnit")) return false;
  if (ignoredDimension !== "project" && filterValue(filters, "project") !== ALL_VALUES.project && item.project !== filterValue(filters, "project")) return false;
  if (ignoredDimension !== "plant" && filterValue(filters, "plant") !== ALL_VALUES.plant && item.plant !== filterValue(filters, "plant")) return false;
  if (ignoredDimension !== "region" && filterValue(filters, "region") !== ALL_VALUES.region && resolveInvestorRegion(item) !== filterValue(filters, "region")) return false;
  if (ignoredDimension !== "salePrice" && filterValue(filters, "salePrice") !== ALL_VALUES.salePrice && salePriceValue(item) !== filterValue(filters, "salePrice")) return false;
  return true;
}

export function buildInvestorFilterOptions(inventory, filters) {
  const compatible = (dimension) => inventory.filter((item) => matchesInvestorFilters(item, filters, dimension));
  ...
  const regions = INVESTOR_REGIONS
    .map((region) => ({ value: region, count: regionCountMap.get(region) ?? 0 }))
  ...
  return {
    ...
    regions,
    salePrices: Array.from(salePriceCountMap, ([value, count]) => ({ value, count }))
...
export function reconcileInvestorFilters(inventory, filters) {
  ...
  const incompatibleDimension = RECONCILIATION_ORDER.find((dimension) => filterValue(filters, dimension) !== ALL_VALUES[dimension]);
  return incompatibleDimension ? { ...filters, [incompatibleDimension]: ALL_VALUES[incompatibleDimension] } : filters;
}
...
export function sortInvestorInventoryBySalePrice(items, direction = "asc") {
  ...
  return [...items].sort((left, right) => {
    ...
    if (leftValid && rightValid && leftPrice !== rightPrice) return (leftPrice - rightPrice) * multiplier;
